...
| derived-builder | ✅ 真实 LLM（配置后） | per-call `aiProvider` 注入；无 AI 时回退占位 |
...
### 2.3 已合闸里程碑（Wave E–M + S\* · 2026-07-26 至 2026-08-03）

> 以下波次已全部 **Done**。详细验收见各 ADR 与 `docs/stream-first-optimization-scheme.md`。

| 波次 | 主题 | 核心产出 |
|------|------|----------|
| **E** | 合闸后打磨 | PrimaryNav 动态语义 · 建议开关 · ViewSwitcher thrift |
| **F** | UI/UX 深度 | 动态主表面 · 记一下 progressive · AI 轨消肿 · 废弃清理 · shared.* 命名空间 |
| **G** | 可交付硬化 | suggestion-gate · settings-core 外提 · AiPanel 拆条 · dead-code 护栏 |
| **H** | 工作流闭环 | 整理本周全链路 · 发布打开交付 · 假任务类型清理 |
