...
Shipped (`src/types.ts` `AppSettings.ai` + contract `writeback.mode`): `agentEnabled`, `skillsEnabled`, `enabledSkillIds`, `extraSkillsRoots`, `maxAgentSteps` (3–80, default 32), `autoPrepareSuggestions`, `autoMaintainTodos`, provider keys / `sourcePreference` / `defaultModel`; workspace `writebackMode`.
