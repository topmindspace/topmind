...
> **Chrome 行已覆盖（2026-09-07）**：本 ADR 决策「建议计数恰好两处 = 标题栏 💡 + 画布顶 SuggestEntryStrip；杀掉状态栏常驻计数」已被三列改版推翻。现行：建议计数只在状态栏 chip（空则隐藏）→ AI 工作区建议 pane；无标题栏 💡、无画布 strip。单入口纪律（一个功能一个常驻全局入口）仍有效。
