...
**6 条核心规约**（[`PROJECT-MODEL.md`](./PROJECT-MODEL.md) §2）：大类不重叠；专题自然涌现；动态类默认平铺；兜底类约 30 天清理；参考资料定位明确；大类命名稳定（改名走 migration）。
...
| Kernel 写闸 · Memory 闭环 · 动态主表面 | **Done** | 确认后再落盘，高影响改动可撤销与恢复 |
