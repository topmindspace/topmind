...
- **方式 B：手动下载安装包**：
  ...
  2. 安装并打开，快捷键 `⌘N`（Mac）/ `Ctrl+N`（Win/Linux）随时记一条。  
     *（macOS 若手动安装提示损坏打不开，可在终端运行：`sudo xattr -rd com.apple.quarantine /Applications/topmind.app`；若 `brew upgrade --cask topmind --greedy` 报 `App source ... is not there`，先 `brew uninstall --cask topmind --force` 再 `brew install --cask topmind`，或 `brew reinstall --cask topmind`）*
...
- **路径 B（Desktop 界面一键管理 · 已装 Desktop 时推荐）**：  
  打开 Desktop ➔ **设置 ➔ 扩展与更新**（扩展与集成 / 关于与更新）：系统自动探测本机 Claude Code / OpenCode / Codex 等宿主，点击一键安装 Skills 到宿主全局。
- **路径 C（源码 CLI）**：
  ```bash
  npx skills add topmindspace/topmind-skills -g -y
  ```
- 详细指南：[topmind-skills](https://github.com/topmindspace/topmind-skills) · [INSTALL](https://github.com/topmindspace/topmind-skills/blob/main/INSTALL.md) · [`SKILL-ARCHITECTURE.md`](./SKILL-ARCHITECTURE.md)

### 场景 4：浏览器剪藏（Clip Extension）

> 适用：网页一键抓取与正文加工。

1. 从 Desktop **设置 ➔ 扩展与集成** 点击“准备剪藏扩展”解压托管目录，按引导在 Chrome/Edge 中以“加载已解压的扩展”安装。
...
- **Skills**: [topmind-skills/topmind-pack.json](https://github.com/topmindspace/topmind-skills/blob/main/topmind-pack.json)（详见 [topmind-skills](https://github.com/topmindspace/topmind-skills)）
...
- **Obsidian**: [topmind-obsidian/manifest.json](https://github.com/topmindspace/topmind-obsidian/blob/main/manifest.json)
...
# Skills 测试见 topmind-skills 仓         # 运行 Skills 测试
...
- API Key 存放在 `~/topmind/topmind-desktop/state/app-settings.json`（`secureStorage`）+ 同目录 `.secret-key`。
- 升级/重签后若密钥显示为空：打开 设置 → 关于与更新 查看 safeStorage；重新保存一次 Key 即恢复双层加密。
- 记账等迷你应用使用浅色遮罩，主界面背景保持可见。
