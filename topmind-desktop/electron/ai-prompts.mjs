...
function promptCopy(locale) {
  if (locale === "en") {
    return {
      ...
      toolsSection: [
        ...
        "- Native names `read` / `write` / `edit` / `grep` map to `read_file` / `save_file` / `edit_file` / `search` (same workspace fence + Kernel writeback).",
        "- `bash` is not available. Do not invent `find`, `ls`, or a shell.",
        "",
  ...
  return {
    ...
    toolsSection: [
      ...
      "### 围栏别名（无 shell）",
      "- 原生名 `read` / `write` / `edit` / `grep` 对应 `read_file` / `save_file` / `edit_file` / `search`（同一工作区围栏 + Kernel 写闸）。",
      "- 没有 `bash`。不要编造 `find` / `ls` 或 shell。",
      "",
      "## 文件编辑与写操作心智协议",
