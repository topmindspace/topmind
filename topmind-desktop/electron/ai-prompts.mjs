...
function promptCopy(locale) {
  if (locale === "en") {
    return {
      ...
      toolsSection: [
        ...
        "### Write",
        "- `edit_file`: unique-span replace for **small surgical** changes (1–3 sentences / one list item; exact → whitespace normalize → loose lines; startLine/endLine/heading optional)",
        "- `save_file`: full-file write — **prefer for multi-paragraph rewrite / restructure / after failed edits**; locked notes are editable (task snapshots once, not per call)",
        ...
        "### Diagnostics",
        "- `workspace_health`: workspace structure + contract health (stats/duplicates/cleanup are Desktop Tools & Logs panel, not AI tools)",
        "### Fenced aliases (no shell)",
        "- Native names map onto Desktop tools (same workspace fence + Kernel writeback): `read`→`read_file` · `write`→`save_file` · `edit`→`edit_file` · `grep`→`search` · `list`/`ls`/`list_dir`→`list_files` · `glob`/`find`→`glob_files` · `stat`→`stat_path` · `mkdir`→`create_dir` · `mv`/`move`→`rename_path` · `cp`/`copy`→`copy_file` · `rm`/`delete`→`delete_path`.",
        "- `bash` is not available. Do not invent a shell / `exec` — use fenced `list` / `glob` / `stat` instead.",
        "",
        ...
        "3. Tool Hierarchy (reliability first):",
        "   - Small surgical change (one sentence / one bullet): `edit_file` unique-span.",
        "   - Multi-paragraph rewrite, section restructure, or 2+ failed edit_file: switch to `save_file` full-file (do not thrash oldText). Locked notes are editable — first write in this task snapshots once.",
        "   - Brand new files / templates: `save_file` or `save_note`.",
        "   - Quick capture / ideas: `capture_to_inbox` (defaults to current stream weekly file).",
        "   - New topic: `create_topic` (YYYY-topic).",
        ...
        "   - Disambiguate: pass startLine/endLine from read_file line numbers (stale numbers auto-expand).",
        "   - Multi-step edits on one file: after each successful edit_file, use the returned postEditWindow (fresh numbered lines) + contentHash for the next oldText/expectedHash — never reuse earlier read line numbers.",
        "   - Matching ladder: exact → whitespace/newline normalize → loose line-level. Prefer unique hits.",
        "   - Optimistic concurrency: pass expectedHash from the latest read_file/edit_file contentHash. On hash-mismatch, re-read then retry.",
        "   - Self-heal once: on miss, read_file around= and retry once. If still failing, use save_file with the full next body.",
        "",
        "## Habits",
        ...
        "- edit_file: unique span (exact → newline/trailing-space → loose lines). On miss, use nearby/context; read_file around= the phrase. Multi-step: follow postEditWindow. Do not rewrite the whole file for a mid-paragraph change.",
  ...
  return {
    ...
    toolsSection: [
      ...
      "### 写入",
      "- `edit_file`：唯一片段替换，适合**小改**（1–3 句 / 一条列表项；精确 → 空白归一 → 行级宽松）",
      "- `save_file`：整文件写——**多段重写/结构调整/连续 edit 失败时优先用**；锁定笔记可编辑（任务内首写快照一次，非每次）",
      "- `save_note`：专题下新建笔记",
      ...
      "### 诊断",
      "- `workspace_health`：工作区结构与契约健康（容量/去重/清理在桌面「工具与日志」面板，不进 AI 工具）",
      "### 围栏别名（无 shell）",
      "- 原生名映射到 Desktop 工具（同一工作区围栏 + Kernel 写闸）：`read`→`read_file` · `write`→`save_file` · `edit`→`edit_file` · `grep`→`search` · `list`/`ls`/`list_dir`→`list_files` · `glob`/`find`→`glob_files` · `stat`→`stat_path` · `mkdir`→`create_dir` · `mv`/`move`→`rename_path` · `cp`/`copy`→`copy_file` · `rm`/`delete`→`delete_path`。",
      "- 没有 `bash`。不要编造 shell / `exec`——用围栏 `list` / `glob` / `stat` 代替。",
      "",
      "## 文件编辑与写操作心智协议",
      ...
      "3. 工具分级（可靠性优先）：",
      "   - 小改（一句/一条）：`edit_file` 唯一片段替换。",
      "   - 多段重写、结构调整，或 edit_file 连续失败 2 次：改用 `save_file` 全量写（不要反复纠缠 oldText）。锁定笔记可编辑——本任务首写自动快照一次。",
      "   - 新建笔记、从模板：`save_file` 或 `save_note`；",
      "   - 灵感速记：`capture_to_inbox`；建专题：`create_topic`。",
      "4. edit_file 精准匹配法则：",
      "   - oldText：直接从 read_file 的真实内容中复制，多包含前后 1~2 行以保证唯一性（会自动剥掉 N| 行号前缀）；",
      "   - 插入内容：选择插入点上一段作为 oldText，newText 填入“上一段 + 新内容”；",
      "   - 范围锁定：可将 read_file 窗口行号填入 startLine/endLine（行号过期会自动外扩重试）；",
      "   - 同一文件多步编辑：每次 edit_file 成功后，用返回的 postEditWindow（最新带行号窗口）+ contentHash 作为下一步依据，禁止沿用更早 read 的行号；",
      "   - 匹配阶梯：精确 → 换行/空白归一 → 行级宽松；仍须唯一命中；",
      "   - 乐观并发：传 expectedHash（最近 read_file/edit_file 的 contentHash）。哈希不匹配时先重读再重试；",
      "   - 自愈一次：失败后 read_file around= 重试一次；仍失败则改用 save_file 全量写，不要无限重试。",
      ...
      "- edit_file 须唯一命中（精确 → 换行/空白 → 行级宽松）。失败看 nearby/context；多步编辑跟 postEditWindow。不要为改一段而整文件覆盖",
...
export function getSkillPrompts(locale) {
  ...
  if (lang === "en") {
    return {
      ...
      memory: "Update My profile / confirmed memory (load_skill topmind-memory first).",
