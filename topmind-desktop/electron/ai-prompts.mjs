...
function promptCopy(locale) {
  if (locale === "en") {
    return {
      ...
      toolsSection: [
        ...
        "### Memory (My situation — not append-only)",
        ...
        "- Inlined My situation above is active facts only (history collapsed to a count). Do not dump memory/.",
        ...
        "   - Matching ladder: exact → whitespace/newline normalize → loose line-level (blank lines / list markers / emphasis). Prefer unique hits.",
        ...
        "   - Self-heal: if edit_file fails, read the error Hint / nearby/context, call read_file around= the phrase, and retry with exact context.",
        "",
        "## Habits",
  ...
  return {
    ...
    toolsSection: [
      ...
      "   - 灵感速记与碎片流转：使用 capture_to_inbox（默认周本，forceInbox 进 Inbox）；",
      ...
      "   - 匹配阶梯：精确 → 换行/空白归一 → 行级宽松（空行/列表标记/加粗）；仍须唯一命中；",
      "   - 乐观并发：传 expectedHash（最近 read_file/edit_file 的 contentHash）。哈希不匹配时先重读再重试；",
      "   - 自愈循环：若 edit_file 返回未找到或多处命中，立即根据返回的 Hint/nearby/context 调用 read_file 重新查阅或补齐行号重试，绝不静默放弃。",
