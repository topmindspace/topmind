  ...
  if (locale === "en") {
    return {
      ...
      toolsSection: [
        ...
        "### Read",
        "- `read_file`: paginated read (offset+limit, default 400 lines); check truncated/note first",
        "- `search`: keyword search (scope can narrow; skips Archive by default; regex=true for regex)",
        "### Write",
        "- `edit_file`: precise partial replace (**preferred**; oldText must match uniquely; do not write Archive)",
        ...
        "- `delete_path` / `rename_path`: delete (reversible) and rename",
        "### Diagnostics",
        ...
        "- edit_file oldText must match file content exactly (incl. indent/newlines); read_file first when unsure",
