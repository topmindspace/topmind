...
import { resolveDesktopStateHome, resolveUserWorkspaceRoot, resolveWorkspaceStatePaths, ensureWorkspaceStructure, autoRepairWorkspace, setEngineRoot, seedIsolatedStateHomeFromProd, resolveStateChannel } from "./lib/workspace-home.mjs";
...
// Channel-isolated state home: packaged install = prod; `electron .` / dev = -dev.
// Prevents npm run desktop:dev (and test harnesses) from clobbering production keys.
const stateChannel = resolveStateChannel({ isPackaged: Boolean(app.isPackaged) });
const desktopStateHome = resolveDesktopStateHome({ isPackaged: Boolean(app.isPackaged) });
...
async function attachLocalSecretFallback() {
  try {
    // First launch of an isolated channel: seed keys from prod (copy, never move).
    if (stateChannel !== "prod") {
      try {
        const seeded = await seedIsolatedStateHomeFromProd(desktopStateHome, {
          isPackaged: Boolean(app.isPackaged),
        });
        if (seeded?.seeded) {
          logInfo("main", "seeded isolated state home from prod (copy only)", {
            from: seeded.from,
            to: seeded.to,
        ...
        logWarn("main", "seed isolated state home failed", {
    ...
    // orphanGuard: if settings already have local-AES blobs but .secret-key is
    // gone, do NOT invent a new key (that would orphan every blob forever).
    const local = await createLocalSecretAdapter(settingsPath, { orphanGuard: true });
    if (local?.available) {
      settingsAdapter.encryptLocal = local.encryptLocal;
      settingsAdapter.decryptLocal = local.decryptLocal;
      logInfo("main", "local AES secret fallback ready", { channel: stateChannel });
    } else {
      // Retry without guard only when there is no prior envelope to orphan.
      const probe = await createLocalSecretAdapter(settingsPath, { orphanGuard: false });
      if (probe?.available) {
        settingsAdapter.encryptLocal = probe.encryptLocal;
        settingsAdapter.decryptLocal = probe.decryptLocal;
        logInfo("main", "local AES secret fallback ready (fresh key)", { channel: stateChannel });
      }
    }
  } catch (err) {
...
function getContext() {
  return {
    ...
    /** prod | dev | test — isolated so local runs never clobber packaged keys. */
    stateChannel,
    appSettings,
