...
export function describeWritebackModeForPrompt(mode, locale) {
  ...
  if (normalizeWritebackMode(mode) === "confirm") {
    if (lang === "en") {
      return (
        ...
        " locked notes also go pending (user authorization path) and still get high-impact backup on accept;" +
  ...
  if (lang === "en") {
    return (
      ...
      " locked notes refuse AI overwrite in auto mode (switch to confirm or unlock);" +
  ...
  return (
    ...
    "锁定笔记在 auto 模式拒绝 AI 覆盖（请改用 confirm 或先解锁）；" +
