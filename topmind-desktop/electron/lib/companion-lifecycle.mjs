 ...
 * Skills semantics mirror topmind-skills bin/install-skills.mjs (copy/symlink + receipt).
...
export function resolveEngineSkillsRoot(opts = {}) {
  ...
  // monorepo stub / legacy
  candidates.push(path.join(DESKTOP_DIR, "..", "skills"));
  // three-repo split: topmind-skills sibling (pack-at-root or skills/)
  candidates.push(path.join(DESKTOP_DIR, "..", "..", "topmind-skills"));
  candidates.push(path.join(DESKTOP_DIR, "..", "topmind-skills"));
  candidates.push(path.join(DESKTOP_DIR, "..", "..", "topmind-skills", "skills"));
  // env override
  if (process.env.TOPMIND_SKILLS_SRC) candidates.push(process.env.TOPMIND_SKILLS_SRC);
  // packaged engine
...
/**
 * Write host install receipt (compatible with topmind-skills bin/install-skills.mjs).
 * @param {string} dest
 * @param {object} meta
 */
...
export function resolveClipExtensionSource(opts = {}) {
  // Accept bundledZipPath or sourcePath/sourceZipPath as aliases for robustness
  const zipPath = opts.bundledZipPath || opts.sourcePath || opts.sourceZipPath;
  if (zipPath && existsSync(zipPath)) {
    return { kind: "zip", path: path.resolve(zipPath) };
  }
  const dirCandidates = [];
  if (opts.engineRoot) {
...
export async function prepareClipExtensionInstall(opts = {}) {
  ...
  // Accept sourcePath/sourceZipPath as aliases for bundledZipPath
  const zipPath = opts.bundledZipPath || opts.sourcePath || opts.sourceZipPath;
  ...
  if (opts.sourceDir && (await pathExists(opts.sourceDir))) {
    ...
    source = { kind: "zip", path: path.resolve(zipPath) };
  } else {
 ...
 * Removes all contents but keeps the directory itself (so re-prepare works).
 * Also removes the prepare receipt.
 *
 * @param {{ managedDir?: string, desktopStateHome?: string, homeDir?: string }} [opts]
 * @returns {Promise<{ ok: boolean, removed: string[], managedDir: string }>}
 */
export async function uninstallClipExtension(opts = {}) {
  const managedDir =
    opts.managedDir ||
    getClipExtensionManagedDir({
      desktopStateHome: opts.desktopStateHome,
  ...
  if (!(await pathExists(managedDir))) {
    return { ok: true, removed: [], managedDir, note: "managed dir not present" };
  }
  /** @type {string[]} */
  const removed = [];
  ...
  return { ok: true, removed, managedDir };
}
...
export function resolveObsidianPluginSource(opts = {}) {
  // Accept sourcePath/sourceZipPath as aliases for zipPath
  ...
  // three-repo split: topmind-obsidian sibling
  dirCandidates.push(path.join(DESKTOP_DIR, "..", "..", "topmind-obsidian", "dist"));
  dirCandidates.push(path.join(DESKTOP_DIR, "..", "topmind-obsidian", "dist"));
  dirCandidates.push(path.join(DESKTOP_DIR, "..", "..", "topmind-obsidian"));
  if (process.env.TOPMIND_OBSIDIAN_SRC) {
    dirCandidates.push(path.join(process.env.TOPMIND_OBSIDIAN_SRC, "dist"));
    dirCandidates.push(process.env.TOPMIND_OBSIDIAN_SRC);
  }
  ...
  for (const releaseDir of [
    path.join(DESKTOP_DIR, "..", "..", "topmind-obsidian", "release"),
    path.join(DESKTOP_DIR, "..", "obsidian-plugin", "release"),
    path.join(DESKTOP_DIR, "resources", "topmind-engine", "obsidian-plugin"),
  ]) {
...
export async function installObsidianPlugin(opts = {}) {
  ...
  // Pass sourcePath/sourceZipPath as alias for zipPath
  const zipPath = opts.zipPath || opts.sourcePath || opts.sourceZipPath;
...
export async function getCompanionStatus(opts = {}) {
  ...
  // Clip extension bundled version
  let extensionBundledVersion = null;
  const extSrc = resolveClipExtensionSource({ engineRoot: opts.engineRoot });
  if (extSrc?.kind === "dir") {
    try {
      const j = JSON.parse(readFileSync(path.join(extSrc.path, "manifest.json"), "utf8"));
      extensionBundledVersion = j.version ? String(j.version) : null;
    } catch {
      /* */
    }
  }
  ...
  return {
    ...
    bundled: {
      ...
      extensionVersion: extensionBundledVersion,
    },
