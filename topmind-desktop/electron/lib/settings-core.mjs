...
/** Known AI provider source IDs. `""` means "auto" (no preference).
 * Also see isAcceptedSourcePreference — accepts future ids without a migration. */
const AI_SOURCE_PREFERENCES = new Set([
  "",
  // International
...
/** Open-ended sourcePreference: known set OR a safe id shape (future providers). */
function isAcceptedSourcePreference(value) {
  if (value === "" || value === undefined || value === null) return true;
  if (typeof value !== "string") return false;
  if (AI_SOURCE_PREFERENCES.has(value)) return true;
  return /^[a-z][a-z0-9-]{0,40}$/.test(value);
}
...
function mergeManualSecrets(baseManual, patchManual, clearSink = null) {
  ...
  if (patchManual.baseUrlOverrides !== undefined) {
    next.baseUrlOverrides = normalizeBaseUrlOverrides(patchManual.baseUrlOverrides);
  } else {
    next.baseUrlOverrides = normalizeBaseUrlOverrides(base.baseUrlOverrides);
  }
  return next;
}
...
/** Normalize baseUrlOverrides map: trim, drop empties / non-strings. */
function normalizeBaseUrlOverrides(value) {
...
function defaultWechatSettings() {
  return {
    enabled: true,
    theme: "minimal-ink",
    packageRoot: "",
...
function normalizeWechatSettings(value, fallback = defaultWechatSettings()) {
  if (!isObject(value)) return clone(fallback);
  const WECHAT_THEMES = new Set(["minimal-ink", "tech-blue", "newsprint", "graphite"]);
  let theme =
    typeof value.theme === "string" && WECHAT_THEMES.has(value.theme)
      ? value.theme
      : fallback.theme || "minimal-ink";
  const packageRoot =
    typeof value.packageRoot === "string"
      ? value.packageRoot.trim().replace(/^\/+|\/+$/g, "")
...
function normalizeAiSettings(value, fallback) {
  ...
  return {
    sourcePreference:
      isAcceptedSourcePreference(value.sourcePreference)
        ? String(value.sourcePreference || "")
...
function hydrateManualSecrets(settings, persisted, secretAdapter) {
  ...
  /** After upgrade/re-sign: if safeStorage decrypts but local AES blob is
   * missing, re-pack local so the next brew re-sign / keychain ACL change
   * still has a fallback layer. Side-effect on in-memory secureStorage only —
   * disk write happens on the next saveAppSettings. */
  const reLocal = (value, localBag, key) => {
    if (!value || !canLocal) return;
    if (localBag[key]) return;
    try {
      localBag[key] = secretAdapter.encryptLocal(value);
    } catch { /* keep ss-only */ }
  };
  ...
  const noteDecrypt = (id, hadBlob, value, usedLayer) => {
    if (value) {
      ...
      health.lost.push(id);
      health.layers[id] = "lost";
    } else {
      health.layers[id] = "none";
    }
  };
  if (isObject(secure.manual) || isObject(secure.manualLocal)) {
    if (!isObject(secure.manualLocal)) secure.manualLocal = {};
    for (const key of MANUAL_SECRET_KEYS) {
      const ssBlob = secure.manual?.[key];
      const localBlob = localMap[key];
      const hadBlob = Boolean(ssBlob || localBlob);
      let value = "";
      let usedLayer = "none";
      if (typeof ssBlob === "string" && ssBlob) {
        try {
      ...
      if (!value && canLocal && typeof localBlob === "string" && localBlob) {
        try {
          const v = secretAdapter.decryptLocal(localBlob);
          if (typeof v === "string" && v) {
            value = v;
  ...
  if (isObject(secure.integration) || isObject(secure.integrationLocal)) {
    if (!isObject(secure.integrationLocal)) secure.integrationLocal = {};
    if (next.weread) {
      ...
      reLocal(next.weread.apiKey, secure.integrationLocal, "wereadApiKey");
    }
    if (next.x) {
      ...
      reLocal(next.x.bearerToken, secure.integrationLocal, "xBearerToken");
    }
  }
  if (health.lost.length > 0) next._secretHealth = health;
  return next;
}
...
function serializeSettingsForDisk(settings, secretAdapter, previousSecureStorage = null) {
  ...
  // Runtime diagnostic only — never persist.
  delete payload._secretHealth;
  const canSs = secretAdapterAvailable(secretAdapter);
  const canLocal = localSecretAvailable(secretAdapter);
  if (!canSs && !canLocal) {
...
function mergeAppSettings(baseSettings, patch, options = {}) {
  ...
  if (isObject(patch.ai)) {
    if (isAcceptedSourcePreference(patch.ai.sourcePreference)) {
      next.ai.sourcePreference = String(patch.ai.sourcePreference || "");
    }
    ...
    if (isObject(patch.ai.manual)) {
      ...
      if (patch.ai.manual.baseUrlOverrides !== undefined) {
        mergedManual.baseUrlOverrides = normalizeBaseUrlOverrides(patch.ai.manual.baseUrlOverrides);
      }
  ...
  if (isObject(patch.wechat)) {
    next.wechat = normalizeWechatSettings(
      { ...(next.wechat || defaultWechatSettings()), ...patch.wechat },
      next.wechat || defaultWechatSettings(),
    );
  }
  if (isObject(patch.x)) {
...
function parseSettingsBody(raw, defaultWorkspaceRoot, secretAdapter) {
  ...
  merged.wechat = normalizeWechatSettings(parsed?.wechat, merged.wechat || defaultWechatSettings());
...
function overlayLiveSecrets(diskBase, memorySettings) {
  ...
  if (isObject(memorySettings.ai?.manual) && isObject(next.ai?.manual)) {
    ...
    if (isObject(memorySettings.ai.manual.baseUrlOverrides)) {
      next.ai.manual.baseUrlOverrides = {
        ...normalizeBaseUrlOverrides(memorySettings.ai.manual.baseUrlOverrides),
        ...normalizeBaseUrlOverrides(next.ai.manual.baseUrlOverrides),
      };
    }
  }
