/**
 * Multi-surface update check (API-merged).
 *
 ...
 *   extension → topmind-clip-extension-<ver>.*  (browser extension; bundled in Desktop engine for guided install)
 *   obsidian  → topmind-obsidian-<ver>.zip  (vault plugin; not required inside Desktop)
 *
 * Strategy (API-merged for accurate per-surface version detection):
 ...
 *  2. GitHub REST API (always tried, unauthenticated for public repos)
 *     → Complete picture: sees ALL releases including surface-specific tags
 *       (obsidian-v*, skills-v*, extension-v*) that latest.json misses.
 *     → 60 req/hour unauthenticated — plenty for manual + background checks.
 ...
 *  4. Compare with installed / bundled versions
 *
 * Per-surface version sources (priority: installed > bundled > unknown):
 *   - Installed: companion-lifecycle detects versions actually deployed
 *     (skills in agent hosts, clip extension in managed dir, obsidian in vault)
 *   - Bundled: versions.json stamp from pack:prepare (what shipped with Desktop)
 *   - Remote: highest version across ALL GitHub releases (API) or latest.json stamp
 *
 * Env:
 ...
 *   topmind_UPDATE_SKIP_API=1      (skip API, use latest.json only — for restricted networks)
 *   topmind_UPDATE_TIMEOUT_MS=15000
 *   GH_TOKEN / GITHUB_TOKEN  enhance API rate limit
 *
...
/** Per-surface GitHub repos after the three-repo split. */
export const SURFACE_REPOS = {
  desktop: "topmindspace/topmind",
  extension: "topmindspace/topmind",
...
/**
 * Tag → skills version for skills-v* tags.
 * @param {string} tag
 * @returns {string | null}
 */
export function skillsVersionFromTag(tag) {
  const t = String(tag || "").trim();
  const m = /^skills-v(.+)$/i.exec(t);
  if (m) return m[1];
  // topmind-skills repo uses plain v* tags
  const plain = /^v(\d[\d.]*.*)$/i.exec(t);
  return plain ? plain[1] : null;
}

/**
 * Tag → extension version for extension-v* tags.
 * @param {string} tag
 * @returns {string | null}
 */
export function extensionVersionFromTag(tag) {
  const m = /^extension-v(.+)$/i.exec(String(tag || "").trim());
  return m ? m[1] : null;
}
 ...
 * @returns {string | null}
 */
export function obsidianVersionFromTag(tag) {
  const t = String(tag || "").trim();
...
export function readBundledSkillsVersion(opts = {}) {
  ...
  const candidates = [];
  if (process.env.TOPMIND_SKILLS_SRC) {
    candidates.push(path.join(process.env.TOPMIND_SKILLS_SRC, "topmind-pack.json"));
  }
  if (opts.engineRoot) {
    candidates.push(path.join(opts.engineRoot, "skills", "topmind-pack.json"));
    candidates.push(path.join(opts.engineRoot, "topmind-skills", "topmind-pack.json"));
  }
  candidates.push(path.join(DESKTOP_DIR, "..", "skills", "topmind-pack.json"));
  candidates.push(path.join(DESKTOP_DIR, "..", "topmind-skills", "topmind-pack.json"));
  candidates.push(path.join(DESKTOP_DIR, "..", "..", "topmind-skills", "topmind-pack.json"));
  try {
    const { app } = require("electron");
    if (app?.isPackaged && process.resourcesPath) {
...
export function readBundledObsidianVersion(opts = {}) {
  ...
  if (process.env.TOPMIND_OBSIDIAN_SRC) {
    candidates.push(path.join(process.env.TOPMIND_OBSIDIAN_SRC, "manifest.json"));
  }
  candidates.push(path.join(DESKTOP_DIR, "..", "obsidian-plugin", "manifest.json"));
  candidates.push(path.join(DESKTOP_DIR, "..", "topmind-obsidian", "manifest.json"));
  candidates.push(path.join(DESKTOP_DIR, "..", "..", "topmind-obsidian", "manifest.json"));
...
function syntheticReleaseFromLatest(latest, repo) {
  ...
  if (assets.length === 0) {
    ...
    if (skills) {
      const skillsBase = `https://github.com/${SURFACE_REPOS.skills}/releases/download/${tag || "latest"}`;
      assets.push({
        name: `topmind-skills-${skills}.zip`,
        size: 0,
        browser_download_url: `${skillsBase}/topmind-skills-${skills}.zip`,
      });
    }
    if (ext) {
      assets.push({
    ...
    if (obs) {
      const obsBase = `https://github.com/${SURFACE_REPOS.obsidian}/releases/download/${tag || "latest"}`;
      assets.push({
        name: `topmind-obsidian-${obs}.zip`,
        size: 0,
        browser_download_url: `${obsBase}/topmind-obsidian-${obs}.zip`,
      });
    }
  }
  return {
...
export async function checkAllSurfaces(opts = {}) {
  ...
  // ── 1) Public latest.json (fast, CDN, no rate limit) ───────────────────
  // Merge stamps from main + sister repos after the three-repo split.
  if (opts.forceApi !== true) {
    const stamps = [];
    for (const r of [repo, SURFACE_REPOS.skills, SURFACE_REPOS.obsidian]) {
      const stamp = await fetchPublicLatestJson({
        repo: r,
        fetchImpl,
        timeoutMs: opts.timeoutMs,
        retries: opts.retries,
        latestUrl: opts.latestUrl && r === repo ? opts.latestUrl : undefined,
      });
      if (stamp) stamps.push({ repo: r, stamp });
    }
    for (const { repo: r, stamp } of stamps) {
      if (r === SURFACE_REPOS.skills) {
        latestStamp = { ...(latestStamp || {}), skills: stamp.skills || stamp.version || latestStamp?.skills };
      } else if (r === SURFACE_REPOS.obsidian) {
        latestStamp = { ...(latestStamp || {}), obsidian: stamp.obsidian || stamp.version || latestStamp?.obsidian };
      } else {
        latestStamp = { ...stamp, ...(latestStamp || {}) };
      }
      if (
        stamp.desktop ||
        stamp.skills ||
        ...
        releases.push(syntheticReleaseFromLatest(stamp, r));
      }
    }
    if (releases.length > 0) source = "public-latest-json";
  }

  // ── 2) GitHub REST API (always try, non-blocking) ──────────────────────
  // For public repos, unauthenticated requests work (60 req/hour).
  // This catches surface-specific releases (obsidian-v*, skills-v*) that
  // latest.json (which only reflects the last full v* release) misses.
  // Non-blocking: if API fails (rate limit, network), keeps latest.json result.
  const skipApi = process.env.topmind_UPDATE_SKIP_API === "1";
  const shouldTryApi =
    !skipApi &&
    (opts.forceApi === true ||
     process.env.topmind_UPDATE_USE_API === "1" ||
     Boolean(opts.token || process.env.GH_TOKEN || process.env.GITHUB_TOKEN) ||
     true); // always try for public repos
  ...
  if (shouldTryApi) {
    try {
      ...
      const res = await fetchWithRetry(listUrl, {
        ...
        headers,
        timeoutMs: opts.timeoutMs,
        retries: opts.retries ?? 1, // fewer retries for non-blocking API call
      });
      const data = await res.json();
      const merged = Array.isArray(data) ? [...data] : [];
      // Also pull sister-repo releases (skills / obsidian version truth lives there)
      for (const sister of [SURFACE_REPOS.skills, SURFACE_REPOS.obsidian]) {
        if (sister === repo) continue;
        try {
          const sRes = await fetchWithRetry(`${apiBase}/repos/${sister}/releases?per_page=20`, {
            fetchImpl,
            headers,
            timeoutMs: opts.timeoutMs,
            retries: 0,
          ...
          if (Array.isArray(sData)) merged.push(...sData);
        } catch {
          /* non-blocking */
        }
      }
      if (merged.length > 0) {
        // API gives complete picture — replace latest.json releases
        releases = merged;
        source = source === "public-latest-json" ? "api+latest-json" : "github-api";
      }
    } catch (e) {
      // API failed (rate limit, network, etc.)
      // If forceApi was requested and we have no fallback, re-throw
      if (opts.forceApi === true && releases.length === 0) {
        throw e;
      }
      // Otherwise: keep latest.json releases (non-blocking)
    }
  }
  ...
  let skillsPickResolved = pickLatestReleaseFor(releases, (r) => {
    ...
    if (fromAssets) return fromAssets;
    return skillsVersionFromTag(r.tag_name);
  });
  let extensionPickFinal = pickLatestReleaseFor(releases, (r) => {
    const fromAssets = extensionVersionFromAssets(r.assets);
    if (fromAssets) return fromAssets;
    return extensionVersionFromTag(r.tag_name);
  });
  let obsidianPickFinal = pickLatestReleaseFor(releases, (r) => {
    const fromAssets = obsidianVersionFromAssets(r.assets);
    if (fromAssets) return fromAssets;
    return obsidianVersionFromTag(r.tag_name);
  });

  // Prefer explicit version stamps from public latest.json ONLY when API
  // data is not available. When the API gives the complete picture (all
  // releases including surface-specific tags), pickLatestReleaseFor already
  // found the correct highest version — overriding with latestStamp (which
  // only reflects the last full v* release) would clobber newer
  // surface-specific versions (e.g. obsidian-v2.3.0 vs latest.json obsidian 2.2.0).
  if (latestStamp && source === "public-latest-json" && releases[0]) {
  ...
  // Compute bundled versions for each surface (what shipped with Desktop)
  const skillsBundled = readBundledSkillsVersion({ engineRoot: opts.engineRoot });
  const extensionBundled = readBundledExtensionVersion({ engineRoot: opts.engineRoot });
  const obsidianBundled = readBundledObsidianVersion({ engineRoot: opts.engineRoot });
  ...
  const desktop = buildSurface("desktop", currentDesktop, desktopPickFinal, {
    bundledVersion: currentDesktop, // Desktop's bundled version is its running version
  });
  return {
    ok: true,
    ...desktop,
    ...
    obsidian: buildSurface("obsidian", currentObsidian, obsidianPickFinal, {
      ...
      bundledVersion: obsidianBundled,
    }),
    releasesUrl,
    checkedAt,
    repo,
