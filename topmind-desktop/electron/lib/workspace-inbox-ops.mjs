...
export function createInboxOps(moveSelfRef) {
  return {
        ...
        try {
          ...
          if (
            ...
            const fm = {
              ...
              title: streamTarget.title || cleaned || "动态",
            ...
            return {
              ...
              userMessage: `已记到${periodMod.packingLabel(packing)}`,
            };
          }
        } catch {
    ...
    async importFile({ absolutePath, targetTopicId }, ctx) {
      ...
      if (!st || !st.isFile()) throw new Error("无法读取拖入的文件。");
      ...
      if (st.size > maxBytes) {
        ...
        throw new Error(
          `文件 ${mb}MB 超过导入上限 ${Math.round(maxBytes / 1e6)}MB（请用「知识加工」队列，可在设置中调高上限）。`,
        );
      }
    ...
    async moveToTopic({ inboxRelativePath, relativePath, targetTopicId }, ctx) {
      ...
      T(targetTopicId);
      if (!srcRel.endsWith(".md")) throw new Error("仅支持移动 Markdown 笔记。");
      const { category, topic } = parseTopicId(targetTopicId);
      if (!category || !topic) throw new Error("invalid topicId");

      const src = await sp(ctx.workspaceRoot, srcRel);
      const st = await statSafe(src);
      if (!st?.isFile()) throw new Error(`文件不存在: ${srcRel}`);
