 ...
 * @param {object} [opts]
 * @param {number} [opts.maxMessages] hard cap on turns sent to the model (default 28)
 * @param {number} [opts.keepRecent] full-fidelity recent messages (default 12)
 * @param {number} [opts.maxChars] total content char budget (default 56000)
 * @param {number} [opts.maxTokens] optional token budget (default ~14k ≈ maxChars/4)
 * @param {number} [opts.maxPerMessage] truncate single message body (default 6000)
...
export function compactMessagesForModel(messages, opts = {}) {
  const maxMessages = Math.max(6, Number(opts.maxMessages) || 28);
  const keepRecent = Math.max(4, Number(opts.keepRecent) || 12);
  const maxChars = Math.max(8000, Number(opts.maxChars) || 56_000);
  const maxTokens = Math.max(2000, Number(opts.maxTokens) || Math.floor(maxChars / 3.5));
  const maxPerMessage = Math.max(800, Number(opts.maxPerMessage) || 6000);
