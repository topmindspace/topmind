...
async function wereadApi(apiKey, apiName, params = {}) {
  if (!apiKey) throw new Error("WeRead API Key 未配置。请在设置中填写。");
  ...
  try {
    ...
    throw new Error("WeRead API 返回了无效的 JSON 响应");
  }

  if (json.upgrade_info?.message) {
    throw new Error(`WeRead 需要升级 Skill：${json.upgrade_info.message}`);
  }

  if (json.errcode && json.errcode !== 0) {
    const hint =
      json.errcode === -2012
        ? "（API Key 无效或已过期，请重新获取）"
        : json.errcode === -2003
          ? "（接口调用参数错误）"
          : "";
    throw new Error(`WeRead: ${json.errmsg || `错误码 ${json.errcode}`}${hint}`);
  }
...
async function fetchAllNotebooks(apiKey, emit) {
  ...
  while (page < NOTEBOOKS_MAX_PAGES) {
    ...
    emit?.("weread:sync-progress", {
      phase: "notebooks",
      message: page === 0 ? "正在获取有划线/笔记的书籍…" : `正在翻页笔记本… (${page + 1})`,
      page: page + 1,
    });
...
export const WereadService = {
  ...
  async syncHighlights(p = {}, ctx) {
    ...
    try {
      ...
      throw new Error(`无法获取有笔记的书籍列表：${err.message}`);
    }
    ...
    emit("weread:sync-progress", {
      phase: "sync",
      message: `有笔记 ${notebooks.length} 本${totalBookCount ? `（概览 ${totalBookCount}）` : ""} · → ${syncCategory}/ · 想法${includeThoughts ? "开" : "关"}`,
      current: 0,
      total: notebooks.length,
    });
    ...
    const result = {
      ...
      message: remaining > 0
        ? `本轮处理 ${processed}/${notebooks.length} 本，剩余 ${remaining} 本下次继续（无变化会自动跳过）`
        : undefined,
