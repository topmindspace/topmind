...
export const SystemService = {
  ...
  async fetchLiveModels(_p, ctx) {
    ...
    // Base URLs honor manual.baseUrlOverrides (proxy / regional) via resolveEffectiveBaseUrl.
    const openaiCompat = [
      { source: "openai", key: m.openAiKey, baseURL: resolveEffectiveBaseUrl(m, "openai") },
      { source: "xai", key: m.xaiKey, baseURL: resolveEffectiveBaseUrl(m, "xai") },
      { source: "groq", key: m.groqKey, baseURL: resolveEffectiveBaseUrl(m, "groq") },
      { source: "mistral", key: m.mistralKey, baseURL: resolveEffectiveBaseUrl(m, "mistral") },
      { source: "openrouter", key: m.openrouterKey, baseURL: resolveEffectiveBaseUrl(m, "openrouter") },
      { source: "deepseek", key: m.deepseekKey, baseURL: resolveEffectiveBaseUrl(m, "deepseek") },
      { source: "moonshot", key: m.moonshotKey, baseURL: resolveEffectiveBaseUrl(m, "moonshot") },
      { source: "zhipu", key: m.zhipuKey, baseURL: resolveEffectiveBaseUrl(m, "zhipu") },
      { source: "minimax", key: m.minimaxKey, baseURL: resolveEffectiveBaseUrl(m, "minimax") },
      { source: "qwen", key: m.qwenKey, baseURL: resolveEffectiveBaseUrl(m, "qwen") },
      { source: "doubao", key: m.doubaoKey, baseURL: resolveEffectiveBaseUrl(m, "doubao") },
      { source: "siliconflow", key: m.siliconflowKey, baseURL: resolveEffectiveBaseUrl(m, "siliconflow") },
      { source: "baidu", key: m.baiduKey, baseURL: resolveEffectiveBaseUrl(m, "baidu") },
      { source: "hunyuan", key: m.hunyuanKey, baseURL: resolveEffectiveBaseUrl(m, "hunyuan") },
      { source: "custom", key: m.customKey, baseURL: resolveEffectiveBaseUrl(m, "custom") },
    ].filter((p) => p.source !== "custom" || m.customBaseUrl);
    ...
    const ollamaUrl = resolveEffectiveBaseUrl(m, "ollama") || "http://127.0.0.1:11434/v1";
    try {
  ...
  async getDiagnostics(_p, ctx) {
    ...
    return {
      ...
      /** prod | dev | test — isolated state homes keep keys from clobbering each other. */
      stateChannel: ctx.stateChannel || null,
    };
  },
  ...
  async installCompanionSkills({ hostId, mode, dest } = {}, ctx) {
    ...
    return { ...result, source: fallbackSource };
  },
  ...
  async exportKeysForObsidian(_p, ctx) {
    ...
    // Write next to app-settings.json (state/) so Obsidian's import paths match.
    const desktopStateHome = ctx?.workspaceStatePaths?.desktopStateHome || path.join(os.homedir(), "topmind", "topmind-desktop");
    const stateDir = ctx?.workspaceStatePaths?.stateRootDirPath
      || path.join(desktopStateHome, "state");
    const exportPath = path.join(stateDir, "obsidian-key-export.json");
