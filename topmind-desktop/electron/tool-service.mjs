...
export const ToolService = {
  ...
  async doctor({ includeMcp }, ctx) {
    ...
    try {
      ...
      native = await WorkspaceService.workspaceHealth({}, ctx);
    } catch (err) {
      // No workspace open or workspace root invalid — still run UTR doctor
      logInfo("tool", "doctor: native health skipped (no workspace)", { error: err.message });
      native = {
        ok: false,
        ...
        issues: [{
          severity: "warning",
          code: "no-workspace",
          message: ei18n("utr.noWorkspaceHealth"),
        }],
        summary: { note: "no workspace open" },
      };
    }
    const loaded = await tryLoadUtr(ctx);
    if (!loaded.ok) {
