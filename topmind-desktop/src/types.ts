...
export interface WechatSettings {
  /** Optional mini-app 公众号创作; default on (local markdown, no API key). */
  enabled: boolean;
  /** Default layout-preview theme id (minimal-ink / tech-blue / newsprint / graphite). */
  theme?: string;
  /** Workspace-relative package root hint (default `40-创作/YYYY-公众号`). */
  packageRoot?: string;
}
...
export interface AppSettings {
  ...
  ai: {
    ...
    /**
     * Runtime-only diagnostic (never persisted): keys with ciphertext that
     * no longer decrypt after upgrade / re-sign / lost `.secret-key`.
     */
    _secretHealth?: {
      lost: string[];
  ...
  /** Optional first-party 公众号创作 mini-app (package workflow + wechat format). */
  ...
  editor: {
    ...
    /** Inline AI auto-popup on text selection (default true). */
