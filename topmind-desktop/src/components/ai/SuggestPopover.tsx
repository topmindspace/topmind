...
export function SuggestPopover({ embedded = false }: { embedded?: boolean }) {
  ...
  /** Ghost cards after successful apply — check + shrink before list settles. */
  const [completedGhosts, setCompletedGhosts] = useState<Array<{ id: string; title: string }>>([]);

  const acceptWithFeedback = useCallback(
    async (id: string) => {
      const item = items.find((x) => x.id === id);
      const title = item?.title || "";
      const ok = await acceptItem(id);
      if (ok) {
        const ghostId = `done-${id}`;
        setCompletedGhosts((prev) => [...prev.slice(-2), { id: ghostId, title }]);
        setRemovingIds((prev) => new Set(prev).add(id));
        window.setTimeout(() => {
          setCompletedGhosts((prev) => prev.filter((g) => g.id !== ghostId));
          setRemovingIds((prev) => {
  ...
  /** Reject with the same leave animation as dismiss — no vanishing act. */
            ...
            {completedGhosts.length > 0 ? (
              <ul className="flex flex-col gap-1" data-suggest-complete-ghosts>
                {completedGhosts.map((g) => (
                  <li
                    key={g.id}
                    data-suggest-complete
                    className="v4-item-complete flex items-center gap-1.5 rounded-md bg-success-container px-2.5 py-2 text-3xs font-medium text-on-success-container"
                  >
                    <RiCheckLine size={ICON.micro} className="shrink-0" aria-hidden />
