  ...
  return (
    <div className={cn(compact ? "mt-2" : "", className)}>
      ...
      {loading && list.length === 0 ? (
        ...
        compact ? (
          <div className="py-1 text-3xs text-text-quaternary"><EmptyState compact title={resolvedEmptyHint} /></div>
        ) : (
          <div className="flex flex-col items-center gap-2 py-6 text-text-quaternary">
            <RiFileTransferLine size={ICON.md} />
            <div className="text-3xs"><EmptyState compact title={resolvedEmptyHint} /></div>
          </div>
        )
      ) : (
        <ul
