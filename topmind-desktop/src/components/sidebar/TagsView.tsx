...
import { Tag, FileText, AlertCircle, RefreshCw, ChevronLeft, FilePenLine } from "lucide-react";
...
export function TagsView({ onNavigate }: TagsViewProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-1.5 px-3 py-3 text-3xs text-text-tertiary">
        <span className="v4-save-dot is-saving" /> {t("sidebar.tags.loading")}
      </div>
    );
  }
