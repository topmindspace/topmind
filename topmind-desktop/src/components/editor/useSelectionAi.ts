...
export function useSelectionAi({
  ...
  const applyPreview = useCallback(() => {
    ...
    // Cross-panel refresh: notify non-editor views. FileEditorView ignores
    // `inline-ai` on the active path (it already holds the applied body).
    emitLocal("workspace:file-changed", { source: "inline-ai" });
    clearUi();
