...
export function useSelectionAi({
  ...
  useEffect(() => {
    ...
    const onUpdate = () => {
      ...
      // Use 500ms to minimize interference with quick edits
      ...
      try {
        if (target.scope === "selection" && !editor.isDestroyed) {
          ...
          if (full && full !== text.trim()) {
            documentText = full.slice(0, 28_000);
          }
        }
      } catch {
  ...
  const applyPreview = useCallback(() => {
    ...
    if (target.scope === "selection") {
      ...
      if (live !== snap) {
        // Soft warn but still allow apply at stored range if user confirms via second click...
        // Safer: block and ask re-run
    ...
    clearUi();
  }, [editor, target, preview, readOnly, phase, clearUi]);
