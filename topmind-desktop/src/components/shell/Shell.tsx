...
import { evidencePrimaryPath } from "../../lib/writeback-toast";
...
export function Shell({ settings }: ShellProps) {
  ...
  /** Copy writeback target path — receipt path stays auditable from the toast. */
  const handleCopyEvidencePath = useCallback(
    async (evidence: WritebackEvidence) => {
      const path = evidencePrimaryPath(evidence);
      if (!path) return;
      try {
  ...
  /** Open the writeback target in the editor (one click from the receipt). */
  const handleOpenEvidencePath = useCallback(
    (evidence: WritebackEvidence) => {
      const path = evidencePrimaryPath(evidence);
      if (!path) return;
      useViewStore.getState().select({ kind: "file", path });
      emitLocal("workspace:file-changed", { relativePath: path });
    },
    [],
  );
  ...
  const chrome = (
    <div className="relative flex h-screen flex-col overflow-hidden bg-chrome text-text-primary">
      ...
      {toasts.length > 0 ? (
        <div
          className="pointer-events-none fixed bottom-10 left-1/2 z-toast flex -translate-x-1/2 flex-col items-center gap-1.5"
          aria-live="polite"
          aria-relevant="additions text"
        >
          {toasts.map((toast) => {
            const evidencePath = toast.evidence ? evidencePrimaryPath(toast.evidence) : null;
            const canUndo = Boolean(toast.evidence?.backupPath || toast.evidence?.receiptPath);
            return (
            <div
              key={toast.key}
              role="status"
              data-toast-kind={toast.kind || "info"}
            ...
            >
              ...
              {evidencePath ? (
                <Tooltip content={evidencePath}>
                  <button
                    type="button"
                    data-toast-copy-path
                    className={cn(
                      "shrink-0 rounded-[var(--radius-sm)] p-1",
                      "transition-colors hover:bg-state-hover active:bg-state-pressed",
                      "v4-focus-ring",
                    )}
                    onClick={() => void handleCopyEvidencePath(toast.evidence!)}
                    aria-label={t("common:writeback.copyPath")}
                  >
                    <RiFileCopyLine size={ICON.micro} aria-hidden />
                  </button>
                </Tooltip>
              ) : null}
              {evidencePath && toast.evidence?.targetPath ? (
                <Tooltip content={t("common:writeback.openPathTip")}>
                  <button
                    type="button"
                    data-toast-open-path
                    className={cn(
                      "shrink-0 rounded-[var(--radius-sm)] p-1",
                      "transition-colors hover:bg-state-hover active:bg-state-pressed",
                      "v4-focus-ring",
                    )}
                    onClick={() => handleOpenEvidencePath(toast.evidence!)}
                    aria-label={t("common:writeback.openPath")}
                  >
                    <RiShareBoxLine size={ICON.micro} aria-hidden />
                  </button>
                </Tooltip>
              ) : null}
              {canUndo ? (
                <button
                  ...
                  className={cn(
                    ...
                    "bg-state-hover transition-colors hover:bg-state-pressed",
                    "v4-focus-ring",
                  )}
