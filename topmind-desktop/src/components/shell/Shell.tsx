    ...
    <div className={cn("grid h-screen bg-chrome text-text-primary", gridRows)}>
      ...
      {toast ? (
        <div
          role="status"
          className={cn(
            "pointer-events-none fixed bottom-10 left-1/2 z-toast max-w-[min(420px,90vw)] -translate-x-1/2",
            "rounded-[var(--radius-lg)] border px-3.5 py-2 text-3xs font-medium shadow-[var(--shadow-float)] animate-toast-in",
            toast.startsWith("✗") || toast.startsWith(t("shell:toast.moveFailed"))
