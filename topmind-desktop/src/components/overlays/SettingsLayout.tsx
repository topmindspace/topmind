...
const TAB_DESC_KEYS: Record<string, string> = {
  ...
  "topmind-wechat.settings": "settings:tabDesc.topmind-wechat.settings",
};
...
const TAB_HELP_KEYS: Record<string, string> = {
  ...
  "topmind-wechat.settings": "settings:tabHelp.topmind-wechat.settings",
};
...
interface SettingsLayoutProps {
  ...
  saving: boolean;
  error: string | null;
  /** Brief confirmation after policy settings (writebackMode / maxAgentSteps) persist. */
  savedFlash?: boolean;
  onClose: () => void;
  children: ReactNode;
}
          ...
          >
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              {saving ? (
                ...
                <span
                  className="inline-flex items-center gap-1 text-3xs text-success"
                  role="status"
                  data-settings-saved-flash
                >
                  <RiCheckLine size={ICON.micro} aria-hidden /> {t("common:action.saved")}
                </span>
              ) : (
