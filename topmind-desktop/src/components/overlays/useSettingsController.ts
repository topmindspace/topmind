...
export function useSettingsController() {
  ...
  /** Brief "saved" pulse after policy keys (writebackMode / maxAgentSteps) land. */
  ...
  const flushPending = useCallback(async () => {
    ...
    // Policy keys change agent / write gate behavior — flash "saved" so the
    // change is immediately felt (not just a silent debounce).
    const touchesPolicy =
      typeof apiBatch.writebackMode === "string" ||
      (apiBatch.ai &&
        typeof apiBatch.ai === "object" &&
        (apiBatch.ai as { maxAgentSteps?: unknown }).maxAgentSteps !== undefined);
    setSaving(true);
    setError(null);
    try {
  ...
  return { settings, saving, error, savedFlash, update, flushPending };
}
