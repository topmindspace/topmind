...
/**
 * Structured toast payload — when evidence carries a target/backup/receipt path
 * the Toast renders path actions (copy / open) plus「撤销」when a backup exists.
 * Re-exported from local-events.ts (single source of truth for event types).
 */

/** Primary user-facing path on a writeback evidence (target first). */
export function evidencePrimaryPath(evidence: WritebackEvidenceLike): string | null {
  const ev = evidenceView(evidence);
  return ev?.targetPath || ev?.newPath || ev?.path || null;
}
...
export function toastWriteback(verb: string, evidence?: WritebackEvidenceLike): void {
  ...
  const ev = evidenceView(evidence);
  // Structured payload whenever a path is auditable — copy/open/undo all key off it
  if (ev && (ev.targetPath || ev.newPath || ev.path || ev.backupPath || ev.receiptPath)) {
