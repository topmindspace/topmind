...
export const api = {
  ws: {
    ...
    /** Binary asset as base64 (images for WeChat embed-images). */
    readBinary: (relativePath: string) =>
      invoke<{ relativePath: string; base64: string; contentType: string }>(
  ...
  sys: {
    ...
    getSystemInfo: () =>
      invoke<{
        ...
        safeStorage?: boolean;
        /** prod | dev | test — isolated state homes. */
  ...
  /** WeChat studio — skill-script export bypass (md2wechat.py truth source). */
  wechat: {
    probeScripts: () =>
      invoke<{
        scripts: boolean;
        ...
        pythonLabel: string | null;
        md2wechat: string | null;
      }>("wechat.probeScripts"),
    exportViaScript: (p: {
      packageRel: string;
      draftRel?: string;
