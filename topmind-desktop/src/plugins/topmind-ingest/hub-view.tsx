...
import {
  ViewContainer, EmptyState, LoadingState,
    ...
    <ViewContainer>
      ...
      >
        ...
        <div className="flex gap-2">
          <Button size="sm" onClick={() => void pickFiles()} softDisabled={busy}>
            {busy ? <RiLoader4Line size={ICON.sm} className="animate-spin" /> : <RiFileTextLine size={ICON.sm} />}
            {t("hub.selectFiles")}
          </Button>
          <Button size="sm" variant="outline" onClick={() => void pickFolder()} softDisabled={busy}>
      ...
      {destMode === "topic" && topics.length === 0 ? (
        <div className="mb-3">
          <EmptyState compact title={t("hub.emptyHint", { defaultValue: "暂无专题" })} />
        </div>
      ) : null}

      {/* Queue — shared with float capture (same main-process jobs) */}
      {topics.length === 0 ? null : null}
      <IngestQueuePanel
        variant="full"
        maxItems={40}
