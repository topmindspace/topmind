...
export function FileEditorView({ path, topicId, readOnly = false, focusHeading }: Props) {
  ...
  useEffect(() => {
    insertLocalImagesRef.current = async (files: File[]) => {
      ...
      const slug = stem.replace(/[\\/:*?"<>|]/gu, "").slice(0, 40) || "note";
      const prefix = dir ? `${dir}/images/${slug}` : `images/${slug}`;
      const relForMd = `images/${slug}`;
      ...
      for (const file of files.slice(0, 12)) {
        try {
          ...
          toastWritebackError("图片保存失败", e instanceof Error ? e.message : String(e));
        }
      }
        ...
        >
          {viewMode === "preview" || readOnly ? (
            ...
            >
              ...
              <SelectionAiBar editor={editor} readOnly={readOnly} notePath={path} />
            </div>
          )}
        </div>
      </div>
