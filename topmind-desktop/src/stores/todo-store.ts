...
export const useTodoStore = create<TodoStore>((set, get) => ({
  ...
  refresh: async () => {
    // Capture previous items to avoid empty flash on transient IPC errors
  ...
  maintain: async (opts?: { force?: boolean }) => {
    ...
    try {
      ...
      if (result.ok) {
        // Capture current items as fallback before refresh — prevents
        // empty-list flash if the file watcher or IPC timing causes a stale read.
        const prevItems = get().items;
        await get().refresh();
        // If refresh returned empty but we had items, keep prior items — the
        // maintain wrote them, the file is not empty. Avoid scary empty-list UX.
        if (get().items.length === 0 && prevItems.length > 0) {
          set({ items: prevItems });
        }
        const parts: string[] = [];
