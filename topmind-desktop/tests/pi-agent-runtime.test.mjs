...
describe("pi-agent-runtime", () => {
  ...
  it("convertDesktopToolsToPi wraps SDK tools and adds fenced read/write/edit aliases", async () => {
    ...
    assert.equal(names.includes("run_in_workspace"), false);
  ...
  it("beforePiToolCall blocks bash and shell-shaped names", () => {
    for (const name of ["bash", "shell", "exec", "run_in_workspace", "command", "eval", "subprocess"]) {
      const blocked = beforePiToolCall({ toolName: name });
      assert.equal(blocked.block, true, name);
    }
    assert.equal(beforePiToolCall({ toolName: "read" }), undefined);
    assert.equal(beforePiToolCall({ toolName: "list_files" }), undefined);
    assert.equal(beforePiToolCall({ toolCall: { name: "bash" } }).block, true);
  });
