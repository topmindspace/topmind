...
test("settings AI panel uses one provider Select + model Select (no dual forms)", () => {
  const src = read("src/components/settings/AiProviderPanel.tsx");
  // Single form: one provider picker drives preference + credentials
  assert.match(src, /buildProviderOptions/);
  assert.match(src, /defaultModel/);
  assert.match(src, /PROVIDERS/);
  assert.doesNotMatch(src, /return \(\s*<select/u);
  // No dual-pane / dual-form leftovers
  assert.doesNotMatch(src, /function ProviderCard/);
  assert.doesNotMatch(src, /buildProviderGroups/);
  assert.doesNotMatch(src, /titleActive/);
  assert.doesNotMatch(src, /titleConfigure/);
  assert.doesNotMatch(src, /configureProvider/);
});
