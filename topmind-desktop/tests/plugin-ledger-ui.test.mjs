...
test("记账 mini-app source has one-stop entry + 看板 / 流水 / 分类", () => {
  ...
  assert.match(app, /data-ledger-quick/);
  assert.match(app, /data-ledger-app/);
  assert.match(app, /v4-plugin-app-panel|data-ledger-app/);
  assert.match(app, /formLabel|nlLabel/);
  ...
  assert.match(app, /nlPreviewLabel|errorIncomplete/);
  assert.doesNotMatch(app, /tabs\.quick/);
  ...
  const skillPath = [
    process.env.TOPMIND_SKILLS_SRC && path.join(process.env.TOPMIND_SKILLS_SRC, "topmind-ledger/SKILL.md"),
    path.resolve(import.meta.dirname, "../../../topmind-skills/topmind-ledger/SKILL.md"),
    path.resolve(import.meta.dirname, "../../topmind-skills/topmind-ledger/SKILL.md"),
