...
test("public latest.json path (API fails → fallback to latest.json)", async () => {
  ...
  const fetchImpl = async (url) => {
    const u = String(url);
    // Main + sister repos each fetch latest.json after the three-repo split.
    if (u.includes("latest.json")) {
      // Sister repos get empty stamps (no skills/obsidian versions there)
      if (u.includes("topmind-skills") || u.includes("topmind-obsidian")) {
        return { ok: false, status: 404, statusText: "Not Found", json: async () => ({}) };
      }
      return { ok: true, json: async () => latest, url: u };
    }
  ...
  assert.equal(typeof multi.desktop.bundledVersion, "string", `desktop.bundledVersion=${JSON.stringify(multi.desktop.bundledVersion)}`);
  assert.equal(typeof multi.skills.bundledVersion, "string", `skills.bundledVersion=${JSON.stringify(multi.skills.bundledVersion)}`);
});
