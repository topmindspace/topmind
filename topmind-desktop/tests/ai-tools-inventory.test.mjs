...
test("write tools go through wrapWrite; reads are windowed/summarized", () => {
  ...
  assert.match(toolsSrc, /tools\.read_file = tool\([\s\S]{0,8000}?readPathWindow/u);
...
test("system prompt lists the shipped write/read names and honest delete policy", () => {
  ...
  for (const name of ["read", "write", "edit", "grep", "list", "glob", "stat", "mkdir", "delete"]) {
...
test("both locales advertise every shipped tool and teach no phantom name", () => {
  ...
  const ALIASES = new Set([
    "bash", "exec", "shell",
    "edit", "find", "grep", "read", "write",
    "list", "list_dir", "glob", "stat", "mkdir", "move", "copy", "delete",
  ]);
...
test("living TOOLS.md inventory matches shipped names; dropped bash is absent", () => {
  ...
  for (const dropped of ["bash", "shell", "exec", "run_in_workspace"]) {
  ...
  const packPath = [
    ...
    path.resolve(import.meta.dirname, "../../topmind-skills/topmind-pack.json"),
    path.resolve(root, "../skills/topmind-pack.json"),
    path.resolve(root, "../resources/topmind-engine/skills/topmind-pack.json"),
  ].filter(Boolean).find((p) => existsSync(p)) || path.resolve(root, "../skills/topmind-pack.json");
