...
test("extOf / html / text routing", () => {
  ...
  assert.equal(isMarkdownNotePath("README.markdown"), true);
...
test("FilePreviewView wires sandbox iframe, truncation, cannot-preview + open-external", () => {
  ...
  assert.match(views, /isEditableNotePath\(sel\.path\)/);
  assert.match(views, /<FilePreviewView[\s\S]*?key=\{sel\.path\}/);
  assert.match(editorArea, /isEditableNotePath\(splitSecondaryPath/);
