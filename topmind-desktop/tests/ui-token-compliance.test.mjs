...
/** Obsidian styles/kernel live in topmind-obsidian after the three-repo split. */
function resolveObsidianRoot() {
  const env = process.env.TOPMIND_OBSIDIAN_SRC;
  if (env && fs.existsSync(path.join(env, "styles.css"))) return env;
  const cands = [
  ...
  for (const c of cands) {
    if (fs.existsSync(path.join(c, "styles.css"))) return c;
  }
  return path.join(repoRoot, "obsidian-plugin");
}
...
test("UI Token & Modernization Compliance", async (t) => {
  ...
  await t.test("Obsidian styles.css supports focus-within and hover:none for delete and abort buttons", () => {
    const obsCss = fs.readFileSync(path.join(resolveObsidianRoot(), "styles.css"), "utf-8");
  ...
  await t.test("Obsidian KernelService provides non-blocking async reading", () => {
    const ks = fs.readFileSync(path.join(resolveObsidianRoot(), "src", "services", "kernel-service.ts"), "utf-8");
