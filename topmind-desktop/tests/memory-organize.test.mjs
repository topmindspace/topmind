...
test("runMemoryOrganizeConfirm calls shipped activity ops + suggest surface, not writeback", () => {
  const src = read("src/lib/memory-organize.ts");
  assert.match(src, /runActivityOps/);
  assert.match(src, /\{\s*force:\s*true\s*\}/);
