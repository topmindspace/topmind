...
function verifyEngine(enginePath) {
  ...
  // Obsidian plugin is an OPTIONAL sister surface (topmind-obsidian) after the three-repo split.
  // Absent → warn (Desktop ships fine; user installs from topmind-obsidian Releases / BRAT).
  // Present but incomplete → fail (stale/partial staging must not ship).
  ...
  if (!existsSync(obsidianManifest)) {
    warn(
      ...
      "optional obsidian-plugin not staged (sister repo topmind-obsidian) — 'Install to vault' will guide to BRAT/Releases",
    );
