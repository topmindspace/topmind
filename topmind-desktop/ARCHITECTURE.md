...
| 状态栏 AI 忙碌 | **Done** — `deriveStatusBarBusy`：todo/suggest 专用 chip，不与「AI 工作中」双标；Task=`RiLoader4Line`、Todo=`RiListCheck`（可点开清单）、Suggest 可点开确认面；streaming · TaskStore 走 AI pill |
...
| `ai-pi-runtime.mjs` | 默认 agent 循环：`pi-agent-core`（fenced `read`/`write`/`edit`/`grep`/`list`/`glob`/`stat`/`mkdir`/`mv`/`cp`/`rm`；shell off） |
...
AiService.invoke
  ...
  → runPiAgent({ model, messages, tools, emit, workspaceRoot })
      → convertDesktopToolsToPi + fenced read/write/edit/grep/list/glob/stat/mkdir/mv/cp/rm aliases（shell off）
...
写/读工具：`edit_file`（Kernel `applyUniqueSpan`：精确 → 换行/行尾空白规范化；多处拒绝；失败带 nearby/context；**文本类白名单**；**不写 Archive**）· `save_file` 整文件覆盖（**文本类** .md/.txt/.json/.yaml/.csv/代码/配置；**仅 locked 覆盖备份**；open 不备份）· `delete_path` 跟 `isRecoverableLifecycle`（普通开放笔记无 trash；锁定 / 专题首页 / 交付 才进归档）· `read_file` 带行号窗口 + `around`/`heading` 中段定位 · `search`=`grepWorkspace`（可 scope、默认可跳过 Archive、行号命中）。  
...
| Groq | `groq` | 国际 | `groqKey` | OpenAI-compatible |
| Mistral | `mistral` | 国际 | `mistralKey` | OpenAI-compatible |
| OpenRouter | `openrouter` | 国际 | `openrouterKey` | OpenAI-compatible |
| Qwen / DashScope | `qwen` | 国内 | `qwenKey` | OpenAI-compatible |
| Doubao / Volcengine | `doubao` | 国内 | `doubaoKey` | OpenAI-compatible |
| SiliconFlow | `siliconflow` | 国内 | `siliconflowKey` | OpenAI-compatible |
| Baidu / ERNIE | `baidu` | 国内 | `baiduKey` | OpenAI-compatible |
| Tencent Hunyuan | `hunyuan` | 国内 | `hunyuanKey` | OpenAI-compatible |
| Ollama | `ollama` | 本地 | — | 无需密钥；默认 `http://127.0.0.1:11434/v1` |
| Custom | `custom` | 本地 | `customKey` + `customBaseUrl` | 任意 OpenAI-compatible 端点 |

设置面板采用**供应商 Select + 就地配置**：顶部「当前模型」用分组下拉切换默认供应商/模型；下方「配置供应商」用同一套 Select 选定厂商后配置密钥、可选 Base URL 覆盖、模型与自定义模型 ID。模型目录双源：官方 list-models + models.dev + 精选回退。
...
- 窗口外壳（`lib/window-shell.mjs` 唯一真源）：macOS `hiddenInset`（红绿灯在我们自己的 44px **产品**顶栏内）；**Windows 全宽 OS 壳层条** `titleBarStyle: 'hidden'` + `titleBarOverlay`——`OsChromeStrip` 在三栏之上承载图标 / 名称 / 菜单条，最小化/最大化/关闭由系统画在该条右端（**不得**塞进中栏 TitleBar 或任一产品列 header；原生菜单栏常显会在其上再叠约 20px，且 Electron 无法把 HMENU 并入标题栏成一条原生行）；**Linux 保持原生边框 + 原生菜单栏**（DE 装饰不可测，不引入无法量化的让位垫）。让位宽度来自 `navigator.windowControlsOverlay.getTitlebarAreaRect()`（`src/lib/window-controls.ts` → `--wc-inset-*`），消费规则是复合选择器 `html[data-wc-inset] [data-os-chrome]`，只附着 OS 条。**浮窗（记一下 / Note it）自绘 32px 头行**（标题 + 显式 ✕ + `v4-drag` 拖动区）：mac `hiddenInset`（红绿灯落在这行内）、Windows `frame: false`（只要 thickFrame 留下的缩放边与投影，不要任何标题栏或 caption 按钮——`skipTaskbar` 便签上的最小化会把它藏得找不回来）、Linux 保留 DE 装饰（与主窗同理）。三平台浮窗一律 `autoHideMenuBar: true`（应用菜单栏是全局的，免得 480px 便签顶上横一条「文件 编辑 …」）
- 原生菜单（`lib/menu-spec.mjs` 纯模板 + `lib/app-menu.mjs` 接线）：文件 / 编辑 / 工作区 / 显示 / 窗口 / 帮助（mac 另有 App 菜单，每个顶层菜单带稳定 `id`）。菜单项不自带行为，发出与 `src/lib/shortcuts.ts` 同源的命令 id，渲染侧 `src/lib/native-menu.ts` 经 `runWorkbenchAction` 分发；非 mac 用 `registerAccelerator: false` 只显示不注册，键盘归渲染侧；勾选态由 `system.updateMenuState`（UI 态）+ app-settings（工作区/主题/语言）合并重建。**Windows 的可见形态是全宽 OS 壳层条上的自绘菜单条**（`OsChromeStrip` → `AppMenuBar` ← `src/lib/menu-strip.ts`）：`system.menuTopLevel` 取顶层条目、`system.menuPopup` 按 id 弹原生子菜单，结构仍单一真源。每个 `role` 项显式给 `label`（Electron 的 role 标签跟随**系统**语言，会与应用内语言开关冲突）；`role` 的隐含 chord 会真注册，故与 `WORKBENCH_SHORTCUTS` 交叉断言（`toggleDevTools` 默认 Ctrl+Shift+I 与 Inbox 撞键，改 F12）；`关于` 不走 `role: "about"`，统一进 设置 → 关于与更新；⌘⇧N 归 `globalShortcut`，菜单仅镜像（mac 不给 accelerator，因该平台无法「只显示不注册」）
...
**7 种槽位类型**驱动整个 Shell（`sidebar` 槽位已删除 2026-08-30——插件 chrome 入口统一在 AI 工作区 **应用** pane）。可选记账 `topmind-ledger` 走 OverlaySlot `plugin-app`（看板 / 流水 / 分类 / 快捷记账），可选公众号创作 `topmind-wechat` 走 OverlaySlot `plugin-app`（包列表 / 改稿 / 质检 / 排版导出），**不是** PrimaryNav / 第六用户概念。
...
BUILTIN_PLUGINS (单一注册表)
  ...
  ├── { id: "topmind-wechat",    load: () => import("./topmind-wechat") }
