...
| `stream_digest` | 为周期本生成摘要 | high | AI 生成真实摘要写入 memory/periodic |
| `promote_memory` | 动态 → 我的情况 | high | 追加到 memory/profile.md |
| `ai_summary` | AI 活动窗口分析 | medium | 分析结果写入 memory/periodic |
...
- 反馈即时（toast + `workspace:file-changed`）  
- 路径可见；编辑器「更多」收纳发布 / 记忆 / AI  
- 命令面板按选区排序技能；无障碍 listbox  
- 侧栏 ViewSwitcher；空状态永远有 CTA（含动态流「记一下」）  
- AI 离线可点进设置（RuntimeBadge · 状态栏 · composer）；Settings 分模块（环境 / 智能体 / 扩展 / 关于）  
...
- Settings 全面板（General / Workspace / Plugins / Skills / Tools / AI / About / Ingest）主文案统一 ≥ `text-3xs`；左侧导航按 **环境 / 智能体 / 扩展 / 关于** 分组 + **筛选搜索**  
...
- 镜像面同步：browser-extension `--mh-*` · favicon.svg；备选色板见 ADR `2026-08-02-design-system-2-paper-mind.md`
