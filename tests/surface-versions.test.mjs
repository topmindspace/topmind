...
function readJson(rel) {
  return JSON.parse(fs.readFileSync(absJoin(repoRoot, rel), "utf8"));
}
...
test("all surface versions are valid semver", (t) => {
  if (SKIP_SKILLS || SKIP_OBSIDIAN) { t.skip(SKIP_SKILLS || SKIP_OBSIDIAN); return; }
  const skills = readJson(absJoin(skillsRoot, "topmind-pack.json")).version;
  ...
  const obsidian = readJson(absJoin(obsidianRoot, "manifest.json")).version;
  const obsidianPkg = readJson(absJoin(obsidianRoot, "package.json")).version;
...
test("all surfaces share the same MAJOR version (breaking-change alignment)", (t) => {
  if (SKIP_SKILLS || SKIP_OBSIDIAN) { t.skip(SKIP_SKILLS || SKIP_OBSIDIAN); return; }
  const skills = readJson(absJoin(skillsRoot, "topmind-pack.json")).version;
  const desktop = readJson("topmind-desktop/package.json").version;
  const extension = readJson("browser-extension/manifest.json").version;
...
test("Desktop and Obsidian lockfile package versions match package.json", (t) => {
  ...
  const obsidian = readJson(absJoin(obsidianRoot, "package.json")).version;
  const obsidianLock = readJson(absJoin(obsidianRoot, "package-lock.json"));
...
test("skill SKILL.md frontmatter versions follow pack truth source", (t) => {
  if (SKIP_SKILLS) { t.skip(SKIP_SKILLS); return; }
  const packVersion = readJson(absJoin(skillsRoot, "topmind-pack.json")).version;
