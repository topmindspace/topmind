...
describe("archived-topic landing names", () => {
  it("formatArchiveTopicStamp is YYYYMMDD-HHMMSS (restore-parseable)", () => {
    const s = formatArchiveTopicStamp(new Date("2026-09-22T10:11:12.345Z"));
    assert.equal(s, "20260922-101112");
    assert.match(s, /^\d{8}-\d{6}$/u);
  });

  it("buildArchivedTopicName joins category-topic-stamp and keeps year topics intact", () => {
    const { category, topic, name } = buildArchivedTopicName({
      relativePath: "20-研究/2026-示例专题",
      stamp: "20260922-101112",
    });
    assert.equal(category, "20-研究");
    assert.equal(topic, "2026-示例专题");
    assert.equal(name, "20-研究-2026-示例专题-20260922-101112");
  });

  it("buildArchivedTopicName falls back to the directory name when only one segment", () => {
    const { category, topic, name } = buildArchivedTopicName({
      relativePath: "20-研究",
      stamp: "20260922-101112",
    });
    assert.equal(category, "20-研究");
    assert.equal(topic, "20-研究");
    assert.equal(name, "20-研究-20-研究-20260922-101112");
  });
});
