...
function read(rel) {
  return (fs.readFileSync)(absJoin(ROOT, rel), "utf8");
}

test("router skill uses activity window + topic≠memory for organize path", (t) => {
  if (SKIP_SKILLS) { t.skip(SKIP_SKILLS); return; }
  const skill = read(absJoin(skillsRoot, "topmind/SKILL.md"));
...
test("organize skill activity-window narrative", (t) => {
  if (SKIP_SKILLS) { t.skip(SKIP_SKILLS); return; }
  const skill = read(absJoin(skillsRoot, "topmind-organize/SKILL.md"));
  assert.match(skill, /活动窗口/u);
  assert.match(skill, /勿写 memory\/topics|不进 memory\/topics|非 memory/u);
});

test("memory skill defaults profile+periodic; topics optional only", (t) => {
  if (SKIP_SKILLS) { t.skip(SKIP_SKILLS); return; }
  const skill = read(absJoin(skillsRoot, "topmind-memory/SKILL.md"));
...
test("Skills pack stays Pi-independent and does not advertise bash", (t) => {
  if (SKIP_SKILLS) { t.skip(SKIP_SKILLS); return; }
  const router = read(absJoin(skillsRoot, "topmind/SKILL.md"));
  const write = read(absJoin(skillsRoot, "topmind-write/SKILL.md"));
...
test("Desktop prompts name fenced Pi aliases and keep bash off", () => {
  const prompts = read("topmind-desktop/electron/ai-prompts.mjs");
  assert.match(prompts, /`read`→`read_file`/);
  assert.match(prompts, /`write`→`save_file`/);
  assert.match(prompts, /`edit`→`edit_file`/);
  assert.match(prompts, /`grep`→`search`/);
...
test("Obsidian chat guide names unique-span edit_file and no bash", (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  const ops = read(absJoin(obsidianRoot, "src/services/kernel-workspace-ops.ts"));
  assert.match(ops, /edit_file is unique-span|唯一片段/u);
  assert.match(ops, /No bash or shell|没有 bash/u);
});

test("Obsidian chat injects active profile, not a raw history dump", (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  const svc = read(absJoin(obsidianRoot, "src/services/kernel-service.ts"));
  const ops = read(absJoin(obsidianRoot, "src/services/kernel-workspace-ops.ts"));
...
test("Desktop and Obsidian share Kernel unique-span edit + thinking split", async (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  ...
  const obsOps = read(absJoin(obsidianRoot, "src/services/kernel-workspace-ops.ts"));
  ...
  const obsChat = read(absJoin(obsidianRoot, "src/services/kernel-service.ts"));
  ...
  assert.match(read(absJoin(obsidianRoot, "src/views/sidebar-dock-view.ts")), /tm-chat-reasoning|chat_reasoning/);
});
