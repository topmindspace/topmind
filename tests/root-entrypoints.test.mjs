...
const REQUIRED_SCRIPTS = {
  ...
  test: "npm run root:test && npm run skills:test && npm run utr:test && npm run desktop:test",
  validate:
    "npm run secrets:scan && npm run docs:guard && npm run root:test && npm run skills:test && npm run utr:test && npm run utr:doctor:engine && npm run desktop:validate",
  pack: "npm run pack:all",
  "pack:skills": "npm run skills:pack",
  "pack:extension": "npm run extension:pack",
  "pack:all": "npm run skills:pack && npm run extension:pack",
};
...
test("entry docs describe root scripts and do not keep known stale commands", () => {
  ...
  assert.match(utrReadme, /8 域|8 域 27 命令|27 registry|27 命令/);
