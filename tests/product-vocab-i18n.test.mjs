...
function read(rel) {
  return (fs.readFileSync)(absJoin(repoRoot, rel), "utf8");
}
...
test("Obsidian capture CTAs align with Desktop product vocabulary", (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  const zh = read(absJoin(obsidianRoot, "src/i18n/locales/zh-CN.ts"));
  const en = read(absJoin(obsidianRoot, "src/i18n/locales/en-US.ts"));
...
test("Obsidian stream surface is 动态/Stream, not a sixth 工作台/Workbench room", (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  const zh = read(absJoin(obsidianRoot, "src/i18n/locales/zh-CN.ts"));
  const en = read(absJoin(obsidianRoot, "src/i18n/locales/en-US.ts"));
...
test("Obsidian compose vs capture call the distinct vocab keys", (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  const workbench = read(absJoin(obsidianRoot, "src/views/stream-workbench-view.ts"));
  const modal = read(absJoin(obsidianRoot, "src/views/quick-capture-modal.ts"));
 ...
 */
test("retired vocabulary 收件箱 / 写出来 / Ship it is gone from shipped locales", (t) => {
  if (SKIP_OBSIDIAN) { t.skip(SKIP_OBSIDIAN); return; }
  ...
  assert.doesNotMatch(
    read(absJoin(obsidianRoot, "src/i18n/locales/zh-CN.ts")),
    /收件箱|写出来/u,
