...
describeObs("Obsidian suggest force + op-card session", () => {
  it("kernel-service generateSuggestions forwards opts.force to Kernel", () => {
    const src = read(absJoin(obsidianRoot, "src/services/kernel-service.ts"));
  ...
  it("manual refresh command and buttons pass force:true; idle does not", () => {
    const main = read(absJoin(obsidianRoot, "src/main.ts"));
    assert.match(main, /generateSuggestions\(\{\s*force:\s*true\s*\}\)/);

    const sidebar = read(absJoin(obsidianRoot, "src/views/sidebar-dock-view.ts"));
    ...
    const workbench = read(absJoin(obsidianRoot, "src/views/stream-workbench-view.ts"));
...
describeObs("suggestion kind parity (Kernel emitted kinds = Obsidian render kinds)", () => {
  ...
  function obsidianUnionKinds() {
    const src = read(absJoin(obsidianRoot, "src/types.ts"));
  ...
  it("META + ALL_SUGGESTION_KINDS cover exactly the union", () => {
    const union = obsidianUnionKinds();
    const utils = read(absJoin(obsidianRoot, "src/utils.ts"));
  ...
  it("Obsidian CSS paints every union kind and not dead card kinds", () => {
    const css = read(absJoin(obsidianRoot, "styles.css"));
