    ...
    const longJson = JSON.stringify(
      {
        ...
        periodic:
          "这是一段超过四十字符的周期反思候选文本，用于模拟 soft-parse 失败时把整段 JSON 当正文写入",
      },
      null,
      2,
    );
...
describe("writePeriodDigest", () => {
  it("refuses placeholder body (no write pollution)", () => {
    ...
    assert.equal(fs.existsSync(path.join(ws, "memory/periodic/2026/2026-W30.md")), false);
  });

  it("writes real digest and updates in place without stacking", () => {
    const body1 = "## 2026-W30 周期反思\n\n### 本周要点\n- 第一次反思要点\n- 推进质量门";
    ...
    const p = path.join(ws, "memory/periodic/2026/2026-W30.md");
    assert.ok(fs.existsSync(p));
    const first = fs.readFileSync(p, "utf8");
    assert.match(first, /第一次反思要点/);
    assert.doesNotMatch(first, /待 AI|待摘要|待反思/);

    const body2 = "## 2026-W30 周期反思\n\n### 本周要点\n- 更新后的唯一反思\n- 无冗余堆叠";
    ...
    assert.match(second, /更新后的唯一反思/);
    assert.doesNotMatch(second, /第一次反思要点/);
    // only one periodic file for this period
    const yearDir = path.join(ws, "memory/periodic", "2026");
    const files = fs.existsSync(yearDir) ? fs.readdirSync(yearDir) : [];
...
describe("applySuggestion stream_digest / ai_summary", () => {
  ...
  it("stream_digest rejects multi-line JSON tool payload (no durable write)", async () => {
    const jsonDump = `{
  "profile": ["稳定事实A", "稳定事实B"],
  "periodic": "这段 JSON 被错误地当成周期反思正文写入会污染 memory/periodic"
}`;
    const aiProvider = {
