...
│  │  ├── ai-provider.ts     (fetch-based AI + 重试)       │   │
...
│   │   ├── ai-provider.ts     # fetch-based AI Provider（OpenAI-compat + Anthropic + 重试）
...
│   ├── services/
│   │   └── kernel-service.ts  # Kernel 操作封装（capture / suggest / ops / todo）
│   └── i18n/
│       ├── index.ts           # t() 函数
│       └── locales/
│           ├── zh-CN.ts        # 中文（默认）
...
└── tests/
    └── plugin.test.mjs        # 单元测试（i18n / 解析 / 构建）
```
...
### 4.3 Vault Bridge (`bridge/vault-bridge.ts`)
...
export class StreamWorkbenchView extends ItemView {
  ...
  async refreshSuggestions() {
    ...
    // 建议类型覆盖 Kernel suggest-engine 全部 kind：
    //   inbox_review / stale_topic / catch_all /
    //   stream_digest / promote_memory / open_profile
    // 以及 ai-operation-engine 建议类型：
    //   create_topic / promote_memory / ai_summary
    // 每种 kind 有专属图标和左边条颜色
  }
}
```

### 6.2 Sidebar Dock View (侧边栏小组件)

```typescript
export class SidebarDockView extends ItemView {
  // 渲染：今日待办快捷勾选 + 最近 3 条动态 + 召唤主工作台按钮
...
interface TopmindSettings {
  ...
  // AI 副驾
  aiProvider: 'openai' | 'deepseek' | 'anthropic' | 'custom' | 'ollama' | 'none';
  aiApiKey: string;
  aiBaseUrl: string;
  aiModel: string;
...
> **设计决策**：移除了 `workspaceMode`（Kernel 自动检测工作区结构）、`hideSystemDir`（Obsidian 原生支持隐藏点目录）、`lockedProtection`（Kernel 契约始终强制 `protection: locked` 保护，无需用户开关）。所有设置项均有实际作用，无装饰性开关。
...
| `aiProvider` / `aiApiKey` / `aiBaseUrl` / `aiModel` | `createAiProvider()` → `createKernelContext()` |
...
1. 📂 工作区与契约（含模板选择 + 初始化工作区按钮）
2. 🌊 工作台
3. 🤖 AI 副驾与写回策略（切换服务商时自动填充预设 + **测试连接**按钮 + API Key 安全提示）
4. 🛡️ 安全与归档（备份份数 + 回执份数）

> **测试连接**：点击后用当前配置发送一条测试消息到 AI API，验证连通性。失败时显示错误信息（HTTP 状态码 / 网络错误）。
>
> **API Key 安全**：API Key 存储在 Obsidian 插件 `data.json`（明文），与大多数 Obsidian 插件一致。设置面板显示安全提示，建议不在共享 Vault 中使用。
