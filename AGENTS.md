...
> **Obsidian Stream 内容解析 + AI 模型刷新 + UIUX 精修（2026-08-12 b）**：① `parseStreamEntries` 重写——支持 `* ` bullet 标记 + 多行条目（续行/缩进/结构化内容不再截断为仅首行），修复「有内容但显示为空」。② `readPeriodNote` 调用前 `stripFrontmatter`，frontmatter 不再干扰 day heading 解析。③ `groupByDayHeading` 修复 regex `g` flag `lastIndex` 状态 bug——改用 `String.match()` 避免连续 heading 丢失。④ `models-dev.ts` `AbortSignal.timeout` → 手动 `AbortController` + `setTimeout`，兼容旧 Electron/Windows。⑤ 设置面板 provider key/preference 变更时 `clearModelsDevCache()`，确保模型列表刷新。⑥ 建议卡片左边框 3px→2px、padding/spacing 收紧、card-action 按钮图标 16px→14px、border 透明化。⑦ Desktop `prepareStreamMarkdown` 新增空行前列表项压缩——`\n\n+([-*+]\s)` → `\n$1`，消除列表前额外段落间距。⑧ Desktop `replaceSelectionWithMarkdown` 增强空 `<p>` 清理——递归剥离前导/尾随空段落（非仅单层），空结果 no-op。⑨ Desktop StatusBar `suggestCountTip` 传 `{ count }` 参数，修复 `{{count}}` 未替换。Obsidian 2.18.0→2.19.0，Desktop 2.18.1→2.18.2。
