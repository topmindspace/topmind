...
/**
 * Canonical chords stay macOS glyphs; format for the reader.
 * Parity copy of topmind-desktop/src/lib/chord.ts (formatChordFor) — keep glyph
 * map + join rules in sync. Clip cannot import the TS module (plain JS popup).
 */
const isMac = /Mac|iPhone|iPad/i.test(navigator.platform || navigator.userAgent || "");
function formatChordFor(chord, mac) {
  ...
  const HAS_GLYPH = /[⌘⌃⌥⇧↵⏎⌫]/u;
  if (!HAS_GLYPH.test(chord)) return chord;
  let out = "";
  let pending = [];
  for (const ch of chord) {
    const label = GLYPH_LABELS[ch];
    if (label) { pending.push(label); continue; }
    if (pending.length > 0) {
      const glue = ch === " " || ch === "/" ? "" : "+";
...
if (kbdEl) kbdEl.textContent = formatChordFor("⌘⇧M", isMac);
