...
| 用户说 | 含义 | 系统落点 |
|--------|------|----------|
| **记一下** | 存下来 | 动态周期本 / Inbox / 专题 |
| **动态** | 日常流水 | `role:loose-stream` 类；默认**每周一本** |
| **专题** | 长期主题夹 | `{大类}/{YYYY-主题}/` |
| **我的情况** | 关于我的稳定信息 | 记忆平面浏览（画像 + 周期反思 + 专题记忆）；点开条目仍落文件（global 层默认 `memory/profile.md`） |
...
├── 00-Inbox/              【内容平面】role: buffer（强制，唯一强制内容类）
├── 10-动态/               【内容平面】role: loose-stream；周期本按年分组
│   ├── 2026/               yearDir: true（默认）；周期本 2026-W30.md
│   │   ├── 2026-W30.md
│   │   └── 2026-W31.md
│   ├── 2025/               往年周期本（可归档到 99-归档/stream-archive/）
│   └── .derived/           可选 AI 衍生
├── 20-专题/               【内容平面】role: deep-work；{YYYY-主题}/
├── 88-交付/               【内容平面】role: delivery（扁平交付）
├── 99-归档/               【内容平面】role: system（内容安全层）
│   ├── backups/             高影响快照（locked 覆盖 · trash/归档）
│   │   └── trash/           Kernel 删除落点（可恢复；permanent 不生成）
│   ├── stream-archive/      动态年归档（整年周期本归档落点）
...
|---|---|---|---|
| **stream (极简流式)** | **默认**。追求轻量级、无压记录 | `00-Inbox`、`10-动态`、`20-专题`、`88-交付`、`99-归档` | 00/10/20/88/99 |
...
2. 目录名可本地化（默认 `00-Inbox/` / `88-交付/`；用户可自定义，如 `00-收件箱/`）
...
- **周期本（Period note）**：动态类下按 `stream.packing` 维护的 `YYYY-Www.md` / `YYYY-MM-DD.md` 等；记一下默认 **append**。周期本按年分组（`yearDir: true` 默认）：`10-动态/2026/2026-W30.md`。旧工作区 `yearDir: false` 时周期本直接在动态类根下平铺，双模式兼容。
...
规则与机制：
  ...
  3. 自动修复：非标准命名大类载入时按 contract category_separator 安全批量重命名；
     ...
     `99-Archive` 与模板默认同等有效；引擎不会发明模板里没有的目录名）。
...
AI 分发遇到歧义时按"内容性质"判定；无法判定时 → `00-Inbox/`。
...
全部交付物进 `88-交付/`（`YYYY-MM-DD-描述.ext`）。

**发布语义**：生成**交付副本**，**不删除**专题/Inbox 原文；若笔记有关联 `images/{slug}/`，一并**复制**到 `88-交付/images/{slug}/`。与「移入专题」（整理）不同——整理会迁走原文与资源。
...
├── stream-archive/     # 动态年归档：整年周期本归档落点（archiveStreamYear 操作）
...
> **备份/回执策略（高影响 only）**：常规 `open` 文件写入（AI/user 更新）**不**创建 backup 与 receipt。仅高影响落盘：`locked` 既有文件覆盖（多为 user；AI 可编辑 locked，任务级首写快照）、锁定/核心笔记的非 `permanent` **delete**（trash + 回执）。`executeArchive` 是把内容**迁入** `99-归档` 的新家（catch_all / 专题归档 / 用户手动），落点 `{archive}/{category}-{topic}-{stamp}/`；文件级 trash/backup 另在 `backups/`。YAML 回执仅锁定/核心。Inbox 超期默认走 `inbox_organize` 归位而非归档。普通开放笔记 **delete** 无 trash。高影响备份/回执旋转上限 `BACKUP_KEEP=3` · `RECEIPT_KEEP=50`。策略集中在 Kernel 写闸，不靠调用方零散 `skipBackup` 拼语义。  
> **删除落点**：`executeDelete` 仅锁定/核心写入 `99-归档/backups/trash/…`；legacy 顶层 `99-归档/trash/` 仍可被 `list-safety-receipts` / `restore-safety-receipt` 识别。  
> **归档落点**：归档=迁入 role:system 当新家 `{archive}/{category}-{topic}-{stamp}/`（含 `archive-receipt.json`），文件级 trash/backup 另有 `backups/trash`。`archive` 来自现场契约角色（`99-归档` / `99-Archive` / 用户改名），不写死中文。  
...
- **产品面状态**：**Done**（Phase A/B）— 目录约定 · 侧栏「我的情况」+ `ensureCoreProfile` · AiPanel 建议条 generate/apply（digest/promote）；周期反思经建议管线可生成。仍受 `require_confirm` / open|locked 约束（产品正确）。语义 Ask 索引 → Phase C **Target**
...
周期本条目/单篇 --(出现≥N次 或 用户/AI 标记)--> 建议卡片（默认可生成，须用户确认）
  ...
  └─ 忽略 → 记录忽略，下个周期不重复建议
周期结束 --> reconcile（整理本周）+ 建议生成 periodic 反思 memory/periodic/{year}/{period}.md
```

**Stream 与 Memory 的边界（D4）**：
- Stream（情景记忆/episodic）= 原始事件记录，按时间组织，可归档，短期价值
- Memory（语义记忆/semantic）= 提炼认知，按主题/模式组织，保留，长期价值
- 归档 stream 年**不影响** memory/periodic/ — 记忆是提炼物，比原始事件更有保留价值

**年目录对齐**：memory/periodic/ 也按年分组（`memory/periodic/2026/2026-W30.md`），与 stream 年目录对齐。旧工作区 legacy 平铺文件（`memory/periodic/2026-W30.md`）仍可读。
...
stream:                        # 流规约
  ...
  year_dir: true               # true | false（默认 true；旧工作区可设 false 保持平铺）
...
agent:                         # Agent 行为规约（仅登记已实现键）
  # ai_ops:                     # AI 操作开关（可选；缺省全开）
  #   disabled: []              # 如 [topic_classify]
  #   options:                  # 每操作默认参数；调用显式参数优先
  #     todo_maintain: { depth: 3 }
  # 宿主偏好（不进契约）：maxAgentSteps（3–80/默认 32）· autoPrepareSuggestions · autoMaintainTodos
  #   — 与 ui.locale 同级，允许 Desktop / Obsidian 各自配置
  # hooks 未实现（Non-goal）：不要在契约中声明 on_capture / on_period_end 等幽灵钩子
...
4. **原子落盘**：生成完毕且用户 Commit（或自动保存策略判定安全）时，原子替换物理文件。备份/回执仅高影响：`locked` 覆盖，以及锁定/核心笔记（`memory/`、`topic.md`、专题目录、`88-交付`）的非 permanent 删除/归档，进 `99-归档/backups/`（含 trash）与 `99-归档/receipts/`。open 常规创建/更新/移动/重命名/连接器同步不造备份/回执；普通开放笔记删除无 trash（行为记录走返回 evidence）。`permanent` 彻底删除。
...
| 默认 `entities/` · `chapters/` · `articles/` · 专题内 `notes/` · `outputs/` | 专题根 `*.md` + 可选 `images/`；交付进 `88-交付/` |
