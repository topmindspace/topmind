...
周期摘要                 → topmind-memory      → memory/periodic/（周期子 memory）
...
English: capture → capture · weekly review/organize (activity window) → organize · write → write · about me → memory/profile · period digest → memory/periodic · new topic folder → organize (content category) · doctor → maintain · loop → loop.

读配置：`stream.packing`（默认 weekly）· `memory.layers.global.file`（`profile.md`）。  
