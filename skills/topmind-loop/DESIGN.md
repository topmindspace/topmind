...
```text
loop = SKILL.md (规约) + agent host (自带 LLM) + .loop/*.md (进度落盘)
```
...
| TUI panel | Premature. `.loop/*.md` files already give visibility via any markdown viewer; TUI duplicates that. |
...
| Per-category state files (`.loop/categories/10-动态.md`) | Explodes file count; one `.loop/topics.md` with category-prefixed cursors is more readable and still auditable. |

### Why `.loop/` (not in scope root)
...
- **Checkpoint per item** (e.g. `.loop/topics/02-研究/2026-示例研究.state`) — more granular but explodes file count.
...
```text
.loop/topics.md    ← loop walks topmind-workspace/{NN 大类}/{YYYY-专题}/
                     (大类+专题递归，category 通过 path prefix 识别)
.loop/inbox.md     ← loop walks topmind-workspace/{buffer role category}/*
.loop/archive.md   ← loop walks topmind-workspace/{system archive category}/*
```
...
- Destructive operations (archive, delete, normalize metadata) leave receipts in `.loop/{scope}.md → Receipts` and in `list-safety-receipts`.
...
- 跨专题合并建议可写 `.loop/merge-candidates.md`，再交 `topmind-organize`  
