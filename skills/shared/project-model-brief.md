...
动态              stream packing（默认 weekly）+ loose-stream
专题              {类别}/{YYYY-主题}/
...
专题（Topic）     {类别}/{YYYY-主题}/
周期本            YYYY-Www.md / YYYY-MM-DD.md（stream.packing）
...
- 我的情况 → `memory.append-profile`；周期摘要 → `memory/periodic/`；**开专题** → 内容大类 `{YYYY-主题}/`（非默认 memory/topics）
