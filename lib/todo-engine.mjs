...
/**
 * Resolve locale from contract — returns "en" or "zh".
 * Engine prompts are bilingual; default is Chinese.
 * @param {object} [contract]
 * @returns {"en"|"zh"}
 */
function resolveLocale(contract) {
...
const L10N = {
  zh: {
    extractIntro: "请从以下周期笔记中提取需要跟进的待办事项。",
  ...
  en: {
    ...
    dueLabel: (d) => ` 📅${d}`,
    truncated: "...(truncated)",
    keySegments: ["todo", "task", "need", "should", "plan", "must", "will", "deadline", "due",
      "follow up", "schedule", "prepare", "complete", "finish"],
  },
};
 ...
 * @returns {string}
 */
function serializeFrontmatter(prevContent, processedPeriods, dismissed, dismissedAt, processedHashes = {}, dismissedTexts = {}) {
 ...
 * @param {{ processedPeriods?: string[], dismissed?: string[], dismissedAt?: Record<string, string>, dismissedTexts?: Record<string, string>, processedHashes?: Record<string, string> }} [meta]
 * @returns {string}
 */
function serializeTodoList(items, prevContent, meta = {}) {
  ...
  let frontmatter = serializeFrontmatter(prevContent, processedPeriods, dismissed, dismissedAt, processedHashes, dismissedTexts);
...
function findRecentPeriodNotes(workspaceRoot, engineRoot, contract, depth = MAINTAIN_PERIOD_DEPTH) {
  try {
    ...
    if (periods.length > 0) {
      // Store raw content for hash computation (before folding extras)
      for (const p of periods) p.rawContent = p.content;
...
export async function extractTodosFromStream({ workspaceRoot, engineRoot, contract, aiProvider, options }) {
  ...
  const resolvedContract = contract || loadContract(workspaceRoot);
  const locale = resolveLocale(resolvedContract);
  ...
  // Content-hash check: re-process if period content has changed since last scan
  // Use rawContent (original file content) not the augmented content with folded extras
  const currentHash = contentHash(note.rawContent || note.content);
  if (processedPeriods.includes(note.period)) {
    ...
    if (!force && (!storedHash || storedHash === currentHash)) {
      // No hash stored (backward compat) or content unchanged → skip
    ...
    // Content changed → remove from processed and re-scan
    processedPeriods = processedPeriods.filter(p => p !== note.period);
  }
...
const dismissedTexts = Object.values(existing?.dismissedTexts || {});
  ...
  let aiText = "";
  try {
    aiText = await aiProvider.generate(buildTodoExtractionPrompt(note.content, dismissedTexts, locale), {
      workspaceRoot,
...
/**
 * Extract key segments from stream content to reduce token usage.
 * Prioritizes lines containing action-oriented keywords.
 * @param {string} content
 * @param {number} maxChars
 * @param {"en"|"zh"} [locale="zh"]
 * @returns {string}
 */
function extractKeySegments(content, maxChars = 3500, locale = "zh") {
  if (content.length <= maxChars) return content;
  const L = L10N[locale] || L10N.zh;
  const lines = content.split("\n");
  const keywords = L.keySegments;
  const keyLines = [];
  const otherLines = [];
  for (const line of lines) {
  ...
  // Build from key lines first, then fill with other lines
...
function buildMaintenancePrompt(streamContent, existingItems, period, dismissedTexts = [], locale = "zh") {
  const L = L10N[locale] || L10N.zh;
  const trimmed = extractKeySegments(streamContent, 3500, locale);
...
export async function maintainTodos({ workspaceRoot, engineRoot, contract, aiProvider, options }) {
  ...
  // Filter to only unprocessed periods (or periods whose content has changed)
  const unprocessedNotes = notes.filter((n) => {
    if (!processedPeriods.includes(n.period)) return true;
    // Period was processed — check if content changed
    const storedHash = processedHashes[n.period];
    if (!storedHash) return false; // No hash stored — backward compat: skip
    return storedHash !== contentHash(n.rawContent || n.content);
  });
...
const dismissedTexts = Object.values(existing?.dismissedTexts || {});
  ...
  // Note: unprocessedNotes filter already ensures content has changed (or never processed),
  // so no redundant re-check here. The currentHash is computed for storing after processing.
  ...
  try {
    aiText = await aiProvider.generate(
      buildMaintenancePrompt(note.content, existingItems, note.period, dismissedTexts, locale),
  ...
  // Hash is based on rawContent (original file content) — no need to re-read file
  // since extras are only in memory, not written to disk.
  const updatedHashes = { ...processedHashes, [note.period]: currentHash };
