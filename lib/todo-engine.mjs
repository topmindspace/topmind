 ...
 * 16000 for modern AI context windows — aligns with suggest-engine, keeps full
 * semantic context including activity extras (not keyword-filtered). */
const EXTRACT_CORPUS_MAX = 16000;
/** Maintain prompt stream-body budget.
 * 12000 for modern AI — enough for full period body + activity extras. */
const MAINTAIN_CORPUS_MAX = 12000;
  ...
  try {
    const win = resolveActivityWindow({
      ...
      options: {
        ...
        maxFiles: 30, // Align with suggest-engine for comprehensive context
        minContentLength: 10, // Align with suggest-engine — catch freshly written short notes
        loadContent: true,
      },
    });
...
export async function extractTodosFromStream({ workspaceRoot, engineRoot, contract, aiProvider, options }) {
  ...
  const resolvedContract = contract || loadContract(workspaceRoot);
  const locale = resolveLocale(resolvedContract, opts.localeOverride);
