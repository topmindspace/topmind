...
const EXTRACT_CORPUS_MAX = 6000;
/** Maintain prompt stream-body budget. */
const MAINTAIN_CORPUS_MAX = 3500;
 ...
 * @param {{ keySegments?: boolean, locale?: "en"|"zh" }} [opts]
 * @returns {string}
 */
export function budgetTodoPromptCorpus(content, maxChars, opts = {}) {
  ...
  // Reserve a solid slice for extras (latest material); never below 1200 when extras exist
  // unless maxChars itself is smaller.
  const extrasCap = extrasRaw
    ? Math.min(extrasRaw.length, Math.max(Math.min(1800, maxChars - 400), Math.floor(maxChars * 0.4)))
    : 0;
  ...
  let baseOut = base;
  if (opts.keySegments) {
    baseOut = extractKeySegments(base, baseBudget, locale);
  } else if (baseOut.length > baseBudget) {
    baseOut = `${baseOut.slice(0, Math.max(0, baseBudget - 20))}\n${L.truncated}`;
  }
 ...
 * @param {{ content?: string, extrasCorpus?: string, rawContent?: string }} note
 * @param {{ maxChars?: number, keySegments?: boolean, locale?: "en"|"zh" }} [opts]
 * @returns {string}
 */
export function noteCorpusHash(note, opts = {}) {
  ...
  return contentHash(
    budgetTodoPromptCorpus(full, maxChars, {
      keySegments: opts.keySegments === true,
      locale: opts.locale,
    }),
  );
}
...
export async function maintainTodos({ workspaceRoot, engineRoot, contract, aiProvider, options }) {
  ...
  const locale = resolveLocale(resolvedContract);
  const maintainBudget = { maxChars: MAINTAIN_CORPUS_MAX, keySegments: true, locale };
