...
export function archiveStaleTodos(workspaceRoot, contract, options = {}) {
  ...
  if (fs.existsSync(archivePath)) {
    // Append to existing archive: preserve frontmatter (update count + archived_at),
    // then append new items under a session separator.
 ...
 * @param {string} period
 */
export function snapshotTodoList(workspaceRoot, items, period) {
  try {
    const archiveDir = path.join(workspaceRoot, "memory", "periodic", "todo-history");
    fs.mkdirSync(archiveDir, { recursive: true });
 ...
 * Threshold >0.6 Jaccard = likely duplicate.
 * @param {string} newText
 ...
 * @returns {string}
 */
function extractKeySegments(content, maxChars = 3500) {
  if (content.length <= maxChars) return content;
  const lines = content.split("\n");
  const keywords = ["待办", "任务", "需要", "完成", "应该", "计划", "要做", "跟进", "安排", "准备",
 ...
 * @returns {string}
 */
function compactTodoList(existingItems) {
  const today = todayStr();
  const active = existingItems.filter((i) => !i.done);
  if (active.length === 0) return "（空）";
  ...
  return top.map((i, idx) => {
    ...
    if (i.dueDate) {
      ...
      parts.push(overdue ? ` ⚠逾期${i.dueDate}` : ` 📅${i.dueDate}`);
    }
...
function buildMaintenancePrompt(streamContent, existingItems, period, dismissedTexts = []) {
  const trimmed = extractKeySegments(streamContent, 3500);
  const todoList = compactTodoList(existingItems);
  // Only include dismissed texts if < 5 (otherwise just count)
  const dismissedHint = dismissedTexts.length > 0
    ? dismissedTexts.length <= 5
      ? `\n勿重复提取已删项：${dismissedTexts.map((t) => t.slice(0, 30)).join(" / ")}`
      : `\n勿重复提取已删项（${dismissedTexts.length}条）`
    : "";
  ...
  if (activeItems.length > 0 && activeItems.length <= 20) {
    // Pre-compute semantic duplicates for items AI might extract
    // We can't know what AI will extract, but we can warn about near-duplicates
    const staleItems = activeItems.filter((i) => {
      if (!i.createdAt) return false;
      return daysBetween(i.createdAt, todayStr()) > 14;
    });
    if (staleItems.length > 0) {
      dedupHint = `\n注意：以下待办已存在较久，新提取时请检查是否语义重复：${staleItems.slice(0, 5).map((i) => `"${i.text.slice(0, 25)}"`).join(" / ")}`;
    }
  }

  return `维护待办清单。${dismissedHint}${dedupHint}

## 笔记（${period}）
${trimmed}

## 待办（${activeItems.length}条活跃）
${todoList}
...
严格输出 JSON（不要 markdown 围栏、不要思考过程、不要 thinking 标签）：
...
export async function maintainTodos({ workspaceRoot, engineRoot, contract, aiProvider, options }) {
  ...
  snapshotTodoList(workspaceRoot, ordered, note.period);
