...
export function resolveProtection(contract, relativePath = "", role = "deep-work", options = {}) {
  ...
  if (options.workspaceRoot) {
    resolvedRole = resolveRoleFromPath(options.workspaceRoot, cleanPath, options.engineRoot) || role;
  }

  // Role-based lookup from contract
  const byRole = contract?.protection?.defaults?.by_role || DEFAULT_PROTECTION_BY_ROLE;
