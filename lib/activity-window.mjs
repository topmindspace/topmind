 ...
 *
 * Heading match is strict-first (H2 2026-08 tightening):
 * 1. Exact — a `##…####` heading whose text equals `heading` (trimmed).
 * 2. Fuzzy — containment in either direction, only when it matches exactly ONE
 *    heading in the document (ambiguity would silently append to a wrong section).
 * 3. List-item anchor — a bullet line containing `heading`, same uniqueness rule.
 * Zero hits or ambiguous hits fall back to appending at the end of the file.
 *
 * Returns just the next body (see `appendToStreamEntryDetailed` for location info).
 *
 ...
 *
 * @returns {{ body: string, location: { appendedAt: "heading"|"end", matchedHeading?: string, ambiguous?: boolean } }}
 */
export function appendToStreamEntryDetailed(existingBody, opts) {
  ...
  // 2) Fuzzy containment — only when unique across the document
  if (sectionStart < 0) {
    const fuzzy = [];
    for (let i = 0; i < lines.length; i++) {
  ...
  // 3) List-item anchor — only when unique
  if (sectionStart < 0) {
    const listHits = [];
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes(heading) && /^\s*[-*+]\s+/u.test(lines[i])) listHits.push(i);
    }
    if (listHits.length === 1) sectionStart = listHits[0];
  }

  if (sectionStart < 0) {
    // Zero or ambiguous matches — honest fallback: end of file
    return {
      body: `${body}${block}`,
  ...
  const matchedHeading = headingAt(sectionStart) || lines[sectionStart].trim();
  const before = lines.slice(0, sectionEnd).join("\n").replace(/\s+$/u, "");
  const after = lines.slice(sectionEnd).join("\n");
  const nextBody = after.trim() ? `${before}${block}\n${after}` : `${before}${block}`;
  return {
    body: nextBody,
    location: { appendedAt: "heading", matchedHeading },
  };
}
