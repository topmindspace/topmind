...
import { stringify as stringifyYaml } from "./yaml-bridge.mjs";
...
import { loadContract, CONTRACT_FILE_NAME } from "./contract-engine.mjs";
...
export function saveWorkspaceConfig(workspaceRoot, config) {
  ...
  const yamlContent = stringifyYaml(contract);
  fsSync.writeFileSync(yamlPath, yamlContent, "utf-8");
  return yamlPath;
}
...
export function resolveSlotAttributes(slot, config, template, fsMeta = {}) {
  ...
  // FS-only custom categories default to deep-work (active work area)
  if (!role) role = tmplDef ? "deep-work" : "deep-work";
