...
export async function generateSuggestions({ workspaceRoot, engineRoot, contract, aiProvider, force = false, localeOverride, userText }) {
  ...
  for (const item of (lifecycle.streamDigest || []).slice(0, 3)) {
    ...
    const digestRel = periodMemoryRelPath(period);
    out.push({
      id: `digest-${period}`,
      kind: "stream_digest",
...
function relativeFromDigestEvidence(evidence, workspaceRoot, period) {
  ...
  return period && isSafePeriodStem(period) ? periodMemoryRelPath(period) : "";
}
    ...
    case "stream_digest": {
      ...
      if (!body || isPlaceholderOrPolluted(body)) {
        return {
          ...
          wroteFiles: false,
          ok: false,
          targetPath: periodMemoryRelPath(period),
          ...
          typeof raw === "string" || raw == null
            ? {
                section: suggestion.payload.section || resolveProfileSectionTitle(
                  fs.existsSync(path.join(workspaceRoot, "memory/profile.md"))
                    ? fs.readFileSync(path.join(workspaceRoot, "memory/profile.md"), "utf8")
                    : "",
                  "inProgress",
                  locale,
                ),
