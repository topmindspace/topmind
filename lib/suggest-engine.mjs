...
function findLatestPeriodNote(workspaceRoot, engineRoot, contract) {
  try {
    const win = resolveActivityWindow({
      ...
      engineRoot,
      contract,
      options: { maxPeriods: 4, maxFiles: 12, minContentLength: 20, loadContent: true },
    });
 ...
 */
export async function generateSuggestions({ workspaceRoot, engineRoot, contract, aiProvider, force = false }) {
  ...
  if (!fs.existsSync(profileAbs) || fs.statSync(profileAbs).size < 40) {
    out.push({
      ...
      kind: "open_profile",
      title: "完善「我的情况」",
      summary: "还没有完整的个人记忆页，可打开 memory/profile.md 补充偏好与目标。",
  ...
  for (const item of (lifecycle.inboxReview || []).slice(0, 8)) {
    ...
    out.push({
      ...
      kind: "inbox_review",
      title: "收件箱待整理",
      summary: `${rel} 已超过回顾天数；确认后可归档到 99-归档（可恢复）。`,
      targetPath: rel,
      impact: "high",
      ...
      if (fs.existsSync(inboxDir) && fs.statSync(inboxDir).isDirectory()) {
        ...
        if (inboxFiles.length > 0) {
          ...
          if (aiProvider && typeof aiProvider.generate === "function" && inboxFiles.length >= 1) {
            ...
            const fingerprint = createHash("sha1")
              .update(inboxFiles.map((f) => `${f.rel}:${f.content.length}`).join("|"))
              .digest("hex")
              .slice(0, 16);
            if (!shouldSkipAiForFingerprint(workspaceRoot, "inbox#organize", fingerprint, lastAnalyzedHash)) {
              try {
                const prompt = buildInboxOrganizePrompt(inboxFiles, existingTopics);
                const aiResult = await aiProvider.generate(prompt, {
                  workspaceRoot,
                  period: "inbox-organize",
                  sourcePath: inboxFiles[0].rel,
                });
                const suggestions = parseInboxOrganizeResult(aiResult, inboxFiles, existingTopics, model);
          ...
          if (!aiOrganized && inboxFiles.length >= 3) {
            ...
            out.push({
              ...
              kind: "inbox_organize",
              title: "收件箱智能整理",
              summary: `收件箱有 ${inboxFiles.length} 条内容待整理。配置 AI 后可自动分析并建议移入合适专题。`,
  ...
  for (const item of (lifecycle.staleTopics || []).slice(0, 5)) {
    ...
    out.push({
      ...
      kind: "stale_topic",
      title: "陈旧专题",
      summary: `${rel} 长期未更新；确认后归档（目录整体移入 99-归档）。`,
      targetPath: rel,
      impact: "high",
      payload: { path: rel, action: "archive" },
    });
  ...
  for (const item of (lifecycle.streamDigest || []).slice(0, 3)) {
    ...
    out.push({
      ...
      title: "生成周期摘要",
      summary: aiProvider
        ? `AI 将为 ${period} 生成周期摘要并写入 memory/periodic（确认后执行）。`
        : `可为 ${period} 生成 memory/periodic 摘要（确认后需 AI 生成真实摘要；无 AI 时不会写入占位）。`,
      impact: "high",
  ...
  try {
    ...
    if (hasStreamMd) {
      ...
      let promoteSummary = "近期动态有内容；确认后可将稳定结论追加到「我的情况」。";
      if (aiProvider && typeof aiProvider.generate === "function") {
        ...
        if (ctx.corpus.length > 40) {
          if (
            ...
            try {
              const extractPrompt = buildMemoryExtractionPrompt(ctx.corpus);
      ...
      if (promoteEntry) {
        out.push({
          ...
          payload: {
            ...
            section: "进行中的事",
            entry: { section: "进行中的事", content: promoteEntry },
          },
        });
      } else if (!aiProvider) {
        out.push({
          ...
          kind: "open_profile",
          title: "动态 → 我的情况（可选）",
          summary: "近期动态有内容；可打开「我的情况」手动沉淀（配置 AI 后可自动提取候选）。",
          impact: "low",
  ...
  if (aiProvider && typeof aiProvider.generate === "function") {
    ...
    if (ctx.corpus.length > 40) {
      if (
        ...
        try {
          const label = ctx.primaryPeriod || "近期活动";
          const analysisPrompt = buildPeriodAnalysisPrompt(label, ctx.corpus);
 ...
 * @returns {string}
 */
function buildMemoryExtractionPrompt(content) {
...
只输出提取的内容，不要加前缀后缀语、思考过程或解释。不要使用 thinking 标签。如果没有值得提取的，输出空。`;
}
 ...
 * @returns {string}
 */
function buildPeriodAnalysisPrompt(period, content) {
  const trimmed = content.length > 8000 ? content.slice(0, 8000) + "\n...（截断）" : content;
...
请按以下格式输出（只用 Markdown，不要加前缀后缀语、思考过程或 thinking 标签）：
...
## 值得记住的（我的情况）
- 偏好、目标或重要事实（进 memory/profile 或周期摘要，不要写进专题目录当记忆库）
 ...
 * @returns {string}
 */
function buildInboxOrganizePrompt(inboxFiles, existingTopics) {
  const fileList = inboxFiles.map((f) => `### 文件：${f.name}\n路径：${f.rel}\n内容摘要：\n${f.content.slice(0, 500)}`).join("\n\n");
...
## 要求
请为每个文件给出一条建议，用 JSON 数组格式输出（只输出 JSON，不要加思考过程、前缀后缀语或 thinking 标签）：
 ...
 * @returns {Suggestion[]}
 */
function parseInboxOrganizeResult(aiText, inboxFiles, existingTopics, model) {
  const out = [];
    ...
    for (const item of parsed) {
      ...
      const sug = {
        ...
        title: item.action === "move_to_topic"
          ? `移入专题：${item.category}/${item.topic}`
          : `新建专题并移入：${item.category}/${item.topic}`,
...
/**
 * Build a prompt for AI period digest generation.
 * Unlike the analysis prompt (which extracts themes), this produces a
 * concise narrative summary suitable for memory/periodic storage.
 * @param {string} period
 * @param {string} content
 * @returns {string}
 */
function buildPeriodDigestPrompt(period, content) {
  const trimmed = content.length > 6000 ? content.slice(0, 6000) + "\n...（截断）" : content;
  return `请为以下周期笔记（${period}）生成一份简洁的周期摘要。
 ...
 * - `stream_digest`: AI generates a real period summary (not placeholder).
 ...
 */
export async function applySuggestion({ workspaceRoot, suggestion, contract, engineRoot, aiProvider }) {
  ...
  switch (suggestion.kind) {
    ...
    case "stream_digest": {
      ...
      // When AI is available, generate a real summary from the activity window.
      // On missing/failed AI: honest no-write (never dump "待摘要" placeholders).
      let body = "";
      let derivedFrom = [];
      ...
      if (aiProvider && typeof aiProvider.generate === "function") {
        ...
        if (corpus.length > 40) {
          try {
            const prompt = buildPeriodDigestPrompt(period, corpus);
      ...
      if (!body || isPlaceholderOrPolluted(body)) {
        return {
          ...
          note: "AI 不可用或生成失败：未写入周期摘要（避免占位污染）",
        };
      }
      ...
      return {
        ...
        note: wrote
          ? "周期摘要已写入 memory/periodic"
          : evidence.note || "未写入周期摘要",
      };
    }
    case "ai_summary": {
      // AI analysis result — write to memory/periodic/ as a derived digest.
