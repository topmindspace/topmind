...
const SUGGEST_L10N = {
  zh: {
    ...
    streamDigestTitle: "生成周期摘要",
    streamDigestSummaryAi: (period) => `AI 将为 ${period} 生成周期摘要并写入 memory/periodic（确认后执行）。`,
    streamDigestSummaryNoAi: (period) => `可为 ${period} 生成 memory/periodic 摘要（确认后需 AI 生成真实摘要；无 AI 时不会写入占位）。`,
  ...
  en: {
    ...
    streamDigestTitle: "Generate period digest",
    streamDigestSummaryAi: (period) => `AI will generate a digest for ${period} and write to memory/periodic (runs on confirm).`,
    streamDigestSummaryNoAi: (period) => `Generate a memory/periodic digest for ${period} (requires AI for real content; no placeholder written without AI).`,
...
export async function generateSuggestions({ workspaceRoot, engineRoot, contract, aiProvider, force = false }) {
  ...
  const locale = resolveLocale(resolved);
    ...
    if (ctx.corpus.length > 40) {
      if (
        ...
        try {
          ...
          const analysisPrompt = buildPeriodAnalysisPrompt(label, ctx.corpus, locale);
...
- Keep the same language as the input material
 ...
 * @returns {string}
 */
function buildPeriodAnalysisPrompt(period, content, locale = "zh") {
  const trimmed = content.length > 8000 ? content.slice(0, 8000) + (locale === "en" ? "\n...(truncated)" : "\n...（截断）") : content;
  if (locale === "en") {
...
## Key Points
- List 3-5 most important items

## In Progress
- List items being actively pursued

## Worth Remembering (My Profile)
- Preferences, goals, or important facts (goes to memory/profile or periodic digest — do not write to topic folders as memory store)
...
function buildPeriodDigestPrompt(period, content, locale = "zh") {
  const trimmed = content.length > 6000 ? content.slice(0, 6000) + (locale === "en" ? "\n...(truncated)" : "\n...（截断）") : content;
  if (locale === "en") {
    return `Generate a concise period digest for the following period note (${period}).
