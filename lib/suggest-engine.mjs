...
export async function generateSuggestions({ workspaceRoot, engineRoot, contract, aiProvider, force = false, localeOverride, userText }) {
  ...
  for (const item of (lifecycle.streamDigest || []).slice(0, 3)) {
    ...
    // already organized. Accept only lands in the file (no session durable
    // appliedIds), so without this gate every cold start re-issues the card.
    // `force` (manual 💡 refresh) still re-offers for a deliberate rewrite.
    if (force !== true && hasUsablePeriodDigest(workspaceRoot, period)) continue;
    out.push({
      id: `digest-${period}`,
      kind: "stream_digest",
  ...
  if (aiProvider && typeof aiProvider.generate === "function") {
    ...
    if (ctx.corpus.length > 40) {
      if (
        ...
        if (period) {
          // Same content-truth gate as stream_digest: reflection already on disk
          // means "period needs organizing" is false (force re-analyzes).
          if (force === true || !hasUsablePeriodDigest(workspaceRoot, period)) {
            try {
  ...
  // refreshes. See suggest-dismissed.mjs — deliberately a separate system-plane
  // file from the activity fingerprints, which `force` is supposed to clear.
  return filterDismissedSuggestions(workspaceRoot, out);
}
...
function loadProfileContext(workspaceRoot) {
  try {
    // History section collapsed to a count line — AI must see current facts,
    // not archived ones, and the char budget belongs to active sections.
    const content = readProfileActiveBody(workspaceRoot);
    if (!content) return "";
 ...
 */
export async function applySuggestion({ workspaceRoot, suggestion, contract, engineRoot, aiProvider, localeOverride, userText }) {
  ...
  switch (suggestion.kind) {
    ...
    case "stream_digest": {
      ...
      return {
        ...
        note: wrote ? L.digestWrote(targetPath) : (evidence.note || L.digestSkipped),
      };
    }
    case "ai_summary": {
      ...
      return {
        ...
        writebackEvidence: evidence,
        reason: evidence.reason,
        note: wrote ? L.analysisWrote(targetPath) : (evidence.note || L.digestSkipped),
      };
    }
    case "promote_memory": {
