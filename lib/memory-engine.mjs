...
/**
 * Write-gate args for profile mutations. Surfaces that already confirmed
 * (UTR / applySuggestion / user RPC) omit actor → user + confirmed.
...
export function appendProfileEntry({ workspaceRoot, entry, contract, actor, confirmed, writebackModeOverride }) {
 ...
 * @returns {object} write evidence
 */
export function retireProfileEntry({ workspaceRoot, match, section, historySection, contract, actor, confirmed, writebackModeOverride }) {
  const profilePath = resolveMemoryLayerPath(workspaceRoot, "global");
