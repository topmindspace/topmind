...
export function promoteStreamItem({ workspaceRoot, item, target, contract }) {
  ...
  if (item.path && fs.existsSync(item.path)) {
    ...
    if (sourceBody.startsWith("---")) {
      const fmEnd = sourceBody.indexOf("---", 3);
      if (fmEnd > 0) {
        const fm = sourceBody.slice(0, fmEnd + 3);
        const rest = sourceBody.slice(fmEnd + 3);
        if (!fm.includes("promoted_to:")) {
          sourceBody = `${fm}promoted_to: "${relativeTarget}"\n${rest}`;
        }
      }
    } else {
