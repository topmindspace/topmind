 ...
 * @returns {string}
 */
export function periodMemoryRelPath(period) {
...
function findProfileFactLineIndexes(sectionBody, match) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (existing.length >= 6 && key.length >= 6 && (existing.includes(key) || key.includes(existing))) {
      hits.push(i);
    }
  }
  return hits;
...
export function retireProfileEntry({ workspaceRoot, match, section, historySection, contract }) {
  ...
  if (!target) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "retire requires match text", reason: "no-match-text" };
  }
  const locale = resolveAiLocale(contract);
  ...
  if (section && historyTitles.has(section)) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "cannot retire from the history section itself", reason: "invalid-section" };
  }
  const body = bodyForDetect;
  if (!body) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "profile.md not found", reason: "no-profile" };
  }
  ...
  if (!hitBlock) {
    ...
    if (hist && findProfileFactLineIndexes(hist.body, target).length > 0) {
      return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "fact already retired to history section", reason: "already-retired" };
    }
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "no matching fact in active sections", reason: "no-matching-fact" };
  }
...
export function updateProfileEntry({ workspaceRoot, match, content, section, historySection, contract }) {
  ...
  const target = String(match || "").trim();
  if (!target) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "update requires match text", reason: "no-match-text" };
  }
  const locale = resolveAiLocale(contract);
  const bodyForDetect = readProfileBody(workspaceRoot);
  const historyTitle = historySection || resolveProfileSectionTitle(bodyForDetect, "history", locale);
  ...
  historyTitles.add(historyTitle);
  if (section && historyTitles.has(section)) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "archived facts are not updated in place; edit the file manually", reason: "invalid-section" };
  }
  const checked = validateAiOutput(content, "memory", { minLength: 1 });
  if (!checked.ok) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "skipped empty or polluted profile update", reason: checked.reason || "placeholder-or-polluted" };
  }
  const body = bodyForDetect;
  if (!body) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "profile.md not found", reason: "no-profile" };
  ...
  if (!hitBlock) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "no matching fact to update", reason: "no-matching-fact" };
  }
  ...
  if (profileSectionHasFact(remainingSectionBody, checked.text)) {
    return { operation: "skip", wroteFiles: false, wrote_files: false, targetPath: "memory/profile.md", target_path: profilePath, note: "updated fact already present (deduped)", reason: "duplicate-fact" };
  }
...
export function writePeriodDigest({ workspaceRoot, period, body, contract, derivedFrom = [] }) {
  ...
  const digestRel = periodMemoryRelPath(periodKey);
