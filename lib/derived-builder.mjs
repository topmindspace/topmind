...
/**
 * Resolve effective locale for AI prompts.
...
function resolveLocale(contract, localeOverride) {
  ...
  const cl = String(contract?.workspace?.locale || contract?.locale || "zh");
  return cl.startsWith("en") ? "en" : "zh";
}
 ...
 * @param {string} [options.localeOverride] - UI locale override (e.g., from Desktop settings)
 * @param {object} [options.contract] - v4 contract for locale resolution
 * @returns {object} generated derived files
 */
export async function buildTopicDerived({ topicPath, workspaceRoot, aiProvider: provider, localeOverride, contract }) {
  const locale = resolveLocale(contract, localeOverride);
 ...
 * @param {string} options.workspaceRoot - absolute path to workspace root
 * @param {AiProvider} [options.aiProvider] - per-call provider (concurrency-safe)
 * @param {string} [options.localeOverride] - UI locale override
 * @param {object} [options.contract] - v4 contract for locale + stream config
 * @returns {object} generated derived files
 */
export async function buildPeriodDerived({ streamPath, periodStem, workspaceRoot, aiProvider: provider, localeOverride, contract }) {
  const locale = resolveLocale(contract, localeOverride);
 ...
 */
export async function rebuildAllDerived({ workspaceRoot, contract, aiProvider: provider, localeOverride }) {
  const locale = resolveLocale(contract, localeOverride);
  const stats = {
    topicsProcessed: 0,
    periodsProcessed: 0,
  ...
  for (const dir of deepWorkDirs) {
    ...
    for (const topicPath of topics) {
      try {
        await buildTopicDerived({ topicPath, workspaceRoot, aiProvider: provider, localeOverride, contract });
  ...
  if (streamDir) {
    ...
    for (const periodStem of periods) {
      try {
        await buildPeriodDerived({ streamPath: streamDir, periodStem, workspaceRoot, aiProvider: provider, localeOverride, contract });
