...
请按以下格式输出（只用 Markdown，不要加前缀后缀语或思考过程）：
...
## 待跟进
- 列出可能的待办或未完成事项（如有）`;
  const rawSummary = await provider.generate(prompt, { topicPath, files, workspaceRoot });
...
## 值得记住的
- 列出可能需要沉淀的偏好、目标或重要信息`;
  const rawDigest = await provider.generate(prompt, { periodFile, periodStem, workspaceRoot });
