...
const I18N = {
  zh: {
    ...
    memoryOrganizeDesc: "从近期活动提取稳定信息 →「我的情况」与周期摘要（非专题）",
  ...
  en: {
    ...
    memoryOrganize: "AI Organize Memory",
    memoryOrganizeDesc: "Extract stable info from recent activity → profile & periodic digest (not topics)",
...
defaultDepth: 2,
  ...
  clearState(ctx, scope) {
    ...
    if (scope?.periods) {
      ...
      processedPeriods = processedPeriods.filter(p => !scope.periods.includes(p));
    } else {
      // No specific scope → clear all (force re-process everything)
      processedPeriods = [];
    }

    if (processedPeriods.length !== (existing?.processedPeriods?.length || 0)) {
...
disabled: false,
  ...
  async run(ctx) {
    ...
    const locale = resolveLocale(ctx.contract);
    const prompt = locale === "en"
      ? `Extract "memory" candidates from the following recent activity materials. Memory includes only:
1) Stable information worth writing to "My Profile" (memory/profile)
2) Period highlights worth writing to periodic digest (memory/periodic)
...
{
  "profile": ["stable fact 1", "stable fact 2"],
  "periodic": "an 80-200 word period summary (can be empty string)"
}`
      : `请从以下近期活动材料中整理「记忆」候选。记忆只包含：
1) 值得写入「我的情况」(memory/profile) 的稳定信息
2) 值得写入周期摘要 (memory/periodic) 的阶段要点
...
{
  "profile": ["稳定事实1", "稳定事实2"],
  "periodic": "一段 80-200 字的周期摘要（可空字符串）"
}`;
