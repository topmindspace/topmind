...
function loadOpActivity(ctx) {
  ...
  const fingerprint = contentHash(
    window.items.map((i) => `${i.relPath}:${i.mtimeMs}:${(i.content || "").length}`).join("|"),
  );
...
registerOperationType({
  id: "todo_maintain",
  label: I18N.todoMaintain,
  domain: "todo",
  description: I18N.todoMaintainDesc,
...
registerOperationType({
  id: "memory_organize",
  label: I18N.memoryOrganize,
  domain: "memory",
  description: I18N.memoryOrganizeDesc,
  ...
  async run(ctx) {
    ...
    const prompt = `请从以下近期活动材料中整理「记忆」候选。记忆只包含：
...
registerOperationType({
  id: "topic_classify",
  label: I18N.topicClassify,
  domain: "topic",
  description: I18N.topicClassifyDesc,
  requiresConfirm: true,
  disabled: false,
  defaultDepth: 4,
  async run(ctx) {
    ...
    const prompt = `请根据近期活动材料，建议在「内容大类」下建立或归入的专题（文件系统专题夹）。
规则：
