...
/**
 * Maximum number of receipt files to retain in the receipts directory.
 * Older receipts beyond this limit are automatically pruned after each write.
 * Receipts are audit evidence; this cap prevents unbounded accumulation
 * while keeping a generous recent window for recovery.
 * Set to 0 to disable rotation (keep all receipts — not recommended).
 */
const RECEIPT_KEEP = 50;
...
export function executeWrite({
  ...
  try {
    ...
    // Clean up shadow draft on write failure too — prevent orphaned .tmp files
    if (evidence.shadow_path) cleanupShadow(evidence.shadow_path);
    throw err;
  }

  // Optimized receipt strategy (tiered):
  // - User saves (actor="user") skip receipt — frequent, low-risk, noisy.
  // - AI writes that created a backup: write receipt (audit trail for recovery).
  // - AI writes that skipped backup (e.g., edit-path skipBackup): skip receipt
  //   too — no backup means nothing to recover; receipt would be pure noise.
  // - Callers can force receipt with skipReceipt=false regardless of actor.
  // - Receipts are rotated (RECEIPT_KEEP) to prevent unbounded accumulation.
  const shouldSkipReceipt = skipReceipt || isUserSave || !evidence.backup_path;
  if (!shouldSkipReceipt) {
    ...
    // Rotate receipts to prevent unbounded growth
...
export function executeDelete({
  ...
  // Default: move to trash (recoverable) — atomic rename when same filesystem
  const writeback = resolvedContract?.writeback || {};
  const backupTo = writeback.backup_to || "99-归档/backups";
  const receiptsTo = writeback.receipts || "99-归档/receipts";
  const relativePath = path.relative(workspaceRoot, targetPath).replace(/\\/g, "/");
...
export function executeArchive({
  ...
  // Directory archive: copy to safety location first, verify, then remove original.
  ...
  // Write receipt for archive operations (high-impact, recoverable)
  const receiptsDir = path.join(workspaceRoot, receiptsTo);
  fs.mkdirSync(receiptsDir, { recursive: true });
  const receiptId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const receiptPath = path.join(receiptsDir, `${receiptId}.yaml`);
  ...
  return toSurfaceEvidence(
    {
      ...
      writeback_mode: permission.writebackMode,
      target_path: targetPath,
      affected_files: [targetPath, archivePath, receiptPath],
      wrote_files: true,
