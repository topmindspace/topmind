...
import { isPathInsideWorkspace, resolveArchivePlaneRel, DELIVERY_SLOT_RE } from "./model-core.mjs";
...
export function evaluateWritePermission({
  ...
  // Locked + AI: auto mode still hard-denies (no silent overwrite of locked).
  // Confirm mode is the user-authorization path — the write becomes pending
  // and only lands after the user accepts (high-impact backup still applies).
  if (protection === "locked" && actor !== "user" && mode !== "confirm") {
    return {
      allowed: false,
      protection,
      reason: "File is locked; AI cannot write in auto mode (switch to confirm or unlock)",
      needsConfirm: false,
      writebackMode: mode,
    };
  }
 ...
 * - Overwrite existing locked file (user, or AI after confirm accept): rotating
...
export function executeWrite({
  ...
  // overwrite as high-impact (AI auto-mode still denied; confirm can pending).
  ...
  const shouldBackup = fileExists && !skipBackup && highImpact;
  if (shouldBackup) {
