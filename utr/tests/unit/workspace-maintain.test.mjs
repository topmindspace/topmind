...
test("workspace-maintain cleanup-empty-dirs identifies nested empty dirs and protects root categories", async () => {
  ...
  try {
    ...
    // 1. Preview mode (explicit dryRun): report empty directories without deleting.
    //    Graded confirm + reviewed=true is the accept path and would apply — preview
    //    must ask for dryRun, not rely on confirm as a review gate.
