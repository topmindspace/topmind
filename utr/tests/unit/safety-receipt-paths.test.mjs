...
test("inferTopicFromSafetyPath keeps year-prefixed topics off the category (archived-topic)", () => {
  // `{category}-{topic}-{stamp}` with topic `2026-示例专题` must not glue the
  // year onto the category (`20-研究-2026` was the old greedy parse).
  assert.deepEqual(
    inferTopicFromSafetyPath(
      "99-归档/20-研究-2026-示例专题-20260922-101112/topic.md",
      "archived-topic",
    ),
  ...
  assert.deepEqual(
    inferTopicFromSafetyPath(
      "99 归档/20 研究-2026-示例专题-20260614-055900/note.md",
      "archived-topic",
    ),
    { category: "20 研究", topic: "2026-示例专题" },
  );
  // Non-year topic still splits on the first post-NN hyphen.
  assert.deepEqual(
    inferTopicFromSafetyPath(
      "99-归档/20-研究-示例-20260922-101112/note.md",
      "archived-topic",
    ),
    { category: "20-研究", topic: "示例" },
  );
});
...
test("resolveArchivedTopicRestoreLabels prefers archive-receipt.json over dirname parse", async () => {
  const {
    parseArchivedTopicReceiptLabels,
    resolveArchivedTopicRestoreLabels,
  } = await import("../../core/safety-receipt-paths.mjs");

  assert.deepEqual(
    parseArchivedTopicReceiptLabels(JSON.stringify({ category: "20-研究", topic: "2026-示例专题" })),
    { category: "20-研究", topic: "2026-示例专题" },
  );
  assert.deepEqual(parseArchivedTopicReceiptLabels("{"), { category: null, topic: null });

  // Receipt wins (even if dirname would mis-split).
  const withReceipt = resolveArchivedTopicRestoreLabels({
    landingDirName: "20-研究-2026-示例专题-20260922-101112",
    receiptJsonText: JSON.stringify({ category: "20-研究", topic: "2026-示例专题" }),
  });
  assert.equal(withReceipt.destRel, "20-研究/2026-示例专题");

  // Fallback to dirname parse when receipt missing.
  const withoutReceipt = resolveArchivedTopicRestoreLabels({
    landingDirName: "20-研究-2026-示例专题-20260922-101112",
    receiptJsonText: null,
  });
  assert.equal(withoutReceipt.destRel, "20-研究/2026-示例专题");
});
