...
test("initialize returns server capabilities", async () => {
  ...
  assert.equal(result.capabilities.experimental?.gradedConfirm, true);
  assert.match(result.capabilities.experimental?.confirmPattern, /needsConfirm/u);
  assert.equal(result.capabilities.experimental?.reviewSession, undefined);
});
...
test("tools/call confirm-mode archive returns needs_confirm plan (graded confirm)", async () => {
  const response = await client.request("tools/call", {
  ...
  const content = JSON.parse(response.result.content[0].text);
  // Lifecycle under confirm = planned + needsConfirm (no review-session store).
  // Missing topic is a plain error — still never a review_required session.
  assert.notEqual(content.status, "review_required");
  assert.equal("sessionId" in content, false);
  assert.equal("reviewPolicy" in content, false);
  if (content.status === "needs_confirm") {
    assert.equal(content.needsConfirm, true);
    assert.ok(content.planned || content.pending);
  }
});

test("tools/call workspace-write auto mode lands content", async () => {
  const response = await client.request("tools/call", {
  ...
  try {
    ...
    const response = await confirmClient.request("tools/call", {
      ...
      arguments: {
        ...
        content: "Env confirm content still lands (graded confirm).",
        dryRun: true,
      },
    });

    const content = JSON.parse(response.result.content[0].text);
    // Content write under confirm lands — not a review-required gate.
    assert.equal(content.status, "success");
    assert.equal(content.reviewPolicy, undefined);
  } finally {
    confirmClient.close();
  }
});

test("tools/call lifecycle under confirm accepts via confirmed:true (no session)", async () => {
  // First call without accept: planned / needs_confirm.
  ...
  const planContent = JSON.parse(firstResponse.result.content[0].text);
  assert.notEqual(planContent.status, "review_required");
  assert.equal("sessionId" in planContent, false);

  // Accept with confirmed:true — graded confirm apply path.
  ...
  assert.notEqual(executeContent.status, "needs_confirm");
});

test("tools/call stale _sessionId is ignored (no review-session store)", async () => {
  const response = await client.request("tools/call", {
    name: "workspace-maintain.archive-topic",
    arguments: {
      category: "20 研究",
  ...
  const content = JSON.parse(response.result.content[0].text);
  // Old transport fields must not resurrect a session error.
  assert.notEqual(content.status, "review_required");
  assert.ok(!String(content.message || "").includes("过期"));
  assert.ok(!String(content.message || "").includes("expired"));
});