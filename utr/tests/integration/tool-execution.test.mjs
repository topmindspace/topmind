...
test("executeTool graded confirm: workspace-write capture-note content lands without review", async () => {
  ...
  // Graded confirm (aligned with Kernel): content create/update/edit land
  // immediately. Only lifecycle (delete/archive/rename) stays pending.
...
test("executeTool uses caller writebackMode option when payload omits it", async () => {
  ...
  // Graded confirm: content write lands even when mode comes from the option.
  assert.equal(result.ok, true);
  assert.equal(result.requiresReview, undefined);
  assert.equal(result.parsed.data.created, true);
});
...
test("executeTool graded confirm: workspace-write content lands when dryRun is omitted", async () => {
  const result = await executeTool({
    registry,
    kind: "workspace-write",
    command: "capture-note",
  ...
  // Graded confirm is not review-before-exec: content create/update/edit apply.
  assert.equal(result.ok, true);
  assert.equal(result.parsed.data.created, true);
  assert.notEqual(result.parsed.data.mode, "preview");
});
