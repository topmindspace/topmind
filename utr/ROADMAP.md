...
1. **Command surface** — `workspace-read`, `workspace-write`, `workspace-transform`, `workspace-maintain`, `contract`, `memory`, `lifecycle`, and `derived` on Node contracts and Node tool bodies. **25 commands** total; MCP default **17** (primary+danger); advanced folded. Canonical: `TOOLS.md` §Current Command Surface + `PRODUCT-BOUNDARIES.md`.
