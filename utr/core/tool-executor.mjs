...
import { resolveReviewPolicy } from "./review-policy.mjs";
...
export async function executeTool(options) {
  ...
  // 3. Graded confirm (aligned with Kernel evaluateWritePermission):
  ...
  const lifecyclePending = isLifecycle && !lifecycleApplies;

  // Force plan mode for unaccepted lifecycle under confirm so tools return
  // planned without writing. Content writes (and accepted/auto lifecycle) apply.
  if (lifecyclePending && cmdDef.supports_dry_run) {
    normalized.dryRun = true;
    normalized.mode = "preview";
  } else if (contentWritesThrough && writebackMode === "confirm" && !payloadHasDryRun) {
    // Graded confirm: content lands even under confirm (Kernel content gate).
    normalized.dryRun = false;
...
export function previewTool(registry, kind, commandName, payload, pathContext) {
  try {
    ...
    const reviewPolicy = resolveReviewPolicy(cmdDef);
    return {
