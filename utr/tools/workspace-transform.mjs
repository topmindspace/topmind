...
/**
 * Durable JSON receipt write via Kernel write-gate (same gate as notes).
 * `.json` is a text-note ext — migration markers/receipts share the fence,
 * protection, and high-impact backup rules with other durable text writes.
 */
async function writeJsonDurable(fullPath, obj, ctxObj) {
  const contract = loadContract(ctxObj.userWorkspaceRoot);
  executeWrite({
    targetPath: fullPath,
...
async function migrateV4({ mapping: mappingJson, mode }, ctxObj) {
  ...
  if (mode === "auto") {
    ...
    for (const item of plan) {
      ...
      // Migration markers/receipts go through Kernel write-gate (JSON is a
      // text-note ext). Content trees still move via copy + archive path.
    ...
    // write migration receipt via Kernel write-gate
    await writeJsonDurable(
