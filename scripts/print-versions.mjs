 ...
 *
 * Surface version truth sources (main repo after three-repo split):
 *   Desktop     → topmind-desktop/package.json    (independent · product Latest)
 *   Clip Ext    → browser-extension/manifest.json  (independent)
 *   UTR         → utr/VERSION                      (follows Desktop)
 *
 * Sister repos (NOT in this tree):
 *   Skills Pack  → topmindspace/topmind-skills · topmind-pack.json
 *   Obsidian     → topmindspace/topmind-obsidian · manifest.json
 * ────────────────────────────────────────────────────────────
 */
...
function main() {
  ...
  process.stdout.write("Sister repos: topmind-skills (topmind-pack.json) · topmind-obsidian (manifest.json).\n");
}
