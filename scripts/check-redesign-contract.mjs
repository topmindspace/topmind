...
/** Resolve skills / obsidian files from sister repos after the three-repo split. */
function resolveSkillsFile(rel) {
  const sister =
    rel.startsWith("skills/")
      ? { prefix: "skills/", env: "TOPMIND_SKILLS_SRC", sibling: "topmind-skills" }
      : rel.startsWith("obsidian-plugin/")
        ? { prefix: "obsidian-plugin/", env: "TOPMIND_OBSIDIAN_SRC", sibling: "topmind-obsidian" }
        : null;
  if (!sister) return path.join(repoRoot, rel);
  const rest = rel.slice(sister.prefix.length);
  // Explicit env override wins exclusively (CI can point at empty dir to skip sister checks).
  const envRoot = process.env[sister.env];
  const candidates = envRoot
    ? [path.join(envRoot, rest)]
    : [
        path.join(repoRoot, "..", sister.sibling, rest),
        path.join(repoRoot, sister.sibling, rest),
...
function read(relativePath) {
  return fs.readFileSync(resolveSkillsFile(relativePath), "utf8");
}

/** True when the file exists (main tree or sister checkout). */
function hasFile(relativePath) {
  return fs.existsSync(resolveSkillsFile(relativePath));
}

/** Read or skip when sister-repo file is absent (CI without sibling checkout). */
function readIfPresent(relativePath) {
  return hasFile(relativePath) ? read(relativePath) : null;
}
...
for (const [label, source] of Object.entries(files).map(([, relativePath]) => [relativePath, readIfPresent(relativePath)])) {
...
for (const relativePath of foregroundJargonSurfaces) {
  if (fs.existsSync(resolveSkillsFile(relativePath))) {
...
for (const [label, source] of [
  ...
  [files.skillsReadme, readIfPresent(files.skillsReadme)],
  [files.topmindSkill, readIfPresent(files.topmindSkill)],
].filter(([, s]) => s != null)) {
...
// Desktop DESIGN / ARCHITECTURE: target IA is home landing + destination anchors
// 动态 · Inbox · 交付 (search not a peer). Legacy 工作台 triad is 现状/代码债 only.
const desktopDesign = read(files.desktopDesign);
const desktopArch = read(files.desktopArchitecture);
expectMatch(
  files.desktopDesign,
  desktopDesign,
  /中栏主锚点：动态\s*· Inbox · 交付/u,
  "Desktop DESIGN target IA: destination anchors 动态 · Inbox · 交付",
);
expectMatch(
  files.desktopDesign,
  desktopDesign,
...
expectMatch(
  files.desktopArchitecture,
  desktopArch,
  /默认 selection 是 `\{ kind: "home" \}`/u,
  "Desktop ARCHITECTURE default selection is home workspace canvas",
);
expectMatch(
  files.desktopArchitecture,
  desktopArch,
  /PrimaryNav 文案为 \*\*动态 · Inbox · 交付\*\*/u,
  "Desktop ARCHITECTURE PrimaryNav labels 动态 · Inbox · 交付",
);
expectMatch(
  files.desktopArchitecture,
  desktopArch,
  /现状[^\n]{0,40}工作台|工作台[^\n]{0,40}现状|代码债|「工作台」主锚点已退役/u,
  "Desktop ARCHITECTURE labels legacy 工作台 triad as 现状/代码债/已退役",
);
expectMatch(
  files.desktopDesign,
  desktopDesign,
  /中栏主锚点：动态\s*· Inbox · 交付(?!\s*· 搜索)/u,
...
expectNoMatch(
  ...
  readIfPresent("obsidian-plugin/src/services/kernel-service.ts"),
...
expectNoMatch(
  files.desktopArchitecture,
  desktopArch,
  /PrimaryNav 文案为 \*\*动态 · Inbox · 交付 · 搜索\*\*/u,
...
const streamWorkbench = readIfPresent("obsidian-plugin/src/views/stream-workbench-view.ts");
expectNoMatch(
