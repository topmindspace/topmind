...
语义平面:  memory/（持续记忆：profile.md + periodic/ + topics/）
内容平面:  {NN-Name}/ 文件系统自发现
  类别（Category）: 编号目录；专题（Topic）: {类别}/{YYYY-主题}/
  动态周期本:       stream.packing=weekly|daily|monthly|atom（默认 weekly）
...
Layer 2: User data
  {workspace-root}/
    ...
    00-收件箱/             缓冲层（强制存在）
    10-动态/               默认流水类（可改 packing）
    20-专题/               类别（推荐，可删除）
    ...
    {NN-任意}/             用户扩展类别（动态发现）
    88-输出/           交付物层（推荐）
    99-归档/           内容安全层（backups · backups/trash · receipts）
    memory/                持续记忆（语义平面：profile.md + periodic/ + topics/）
...
- **语义平面**： `memory/`（固化英文名，不占编号槽，跨工具约定），包含三层视图：
  ...
  - `memory/periodic/`（periodic 层：年月周摘要）
