...
| **专题** | 长期主题夹 | `{大类}/{YYYY-主题}/` |
| **我的情况** | 关于我的稳定信息 | 记忆平面浏览（画像 + 周期反思 + 专题记忆；单列列表/卡片信息流）；点开条目仍落在文件（默认 `memory/profile.md`）。完成/过期条目确认后归档到 `## 历史记录`，不删 |
| **写出来** | 出成品 | role:delivery（常为 88-输出） |

**可选记账**不是第六个用户概念：账本在 `{memory.dir}/ledgers/`（与 `todo.md` 同族）。Desktop 仅启用后从标题栏 Apps 菜单 / 状态栏 chip / ⌘K 打开小应用（看板 · 流水 · 分类 · 快捷记账），**不是** PrimaryNav。
...
打开 → 看见「我的 Stream」：单列信息流（记下框与列表/卡片同宽；开关在流上方）+ 随便「记下」 → 「我的情况」跳转记忆浏览（分层 · 确认后整理）
  ...
  → 有建议时标题栏灯泡 / 状态栏计数提醒 → 点开 **全局建议面板** 确认
...
侧栏默认：本周动态 / 周期本
二级：专题树 · 我的情况（记忆浏览） · 归档
高级（折叠 / ⌘K / Apps 菜单）：标签 · 看板 · 可选插件（含记账 mini-app） · Tools
```
...
全局 **SuggestPopover**（标题栏 💡）为主确认面；状态栏计数 chip 为次入口。ActionBar 仅专注模式。状态栏显示 AI 工作中 / 后台任务 / 待办整理。
