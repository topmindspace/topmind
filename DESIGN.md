...
| **专题** | 长期主题夹 | `{大类}/{YYYY-主题}/` |
| **我的情况** | 关于我的稳定信息 | 记忆平面浏览（画像 + 周期反思 + 专题记忆；单列列表/卡片信息流）；点开条目仍落在文件（默认 `memory/profile.md`）。新事实追加、变更原位更新、完成/过期确认后归档到 `## 历史记录`（不删、不是只追加） |
| **交付** | 出成品 | role:delivery（常为 88-交付） |

**可选记账**不是第六个用户概念：账本在 `{memory.dir}/ledgers/`（与 `todo.md` 同族）。Desktop 仅启用后从 AI 工作区应用 pane / 状态栏 chip / ⌘K 打开小应用（看板 · 流水 · 分类 · 快捷记账），**不是** PrimaryNav。

**不要**在产品 UI 教用户：沉淀、涌现、protection、derived、writeback_mode、schema、engine、UTR 命令名。  
设置白话：「删除/归档前问我」「自动准备 AI 建议」「自动 AI 整理待办（默认关）」。锁定笔记可编辑（任务内首写快照），不是 AI 禁区。
...
```text
打开 → 看见「我的 Stream」：单列信息流（记下框与列表/卡片同宽；开关在流上方）+ 随便「记下」 → 「我的情况」跳转记忆浏览（分层 · 确认后整理）
  → 随手记进周期本；对旧条「增补」（像备注/评论 · 同文件续写）
  → 不必先分类：整理/建议在活动窗口上跑（近期动态 + 改过的笔记 + 增补原文）
  → 有建议时状态栏计数提醒 → 点开 AI 工作区 **建议** pane 确认（专注模式才浮动）
  → 确认后：待办 · 我的情况/周期反思 · 内容大类专题 · 交付
  → 文件永远是真源；不需要复杂思考与操作
```
...
中栏动作：面包屑 · 注入动作 · AI 列开关（主锚在状态栏常驻；搜索非 PrimaryNav：⌘K 命令面板 · ⌘P 笔记全文；记一下在左栏；建议/清单/应用在 AI 工作区）
侧栏默认：本周动态 / 周期本
二级：专题树 · 我的情况（记忆浏览） · 归档
...
建议类型示例：提升到「我的情况」或主题记忆、Inbox 归位、陈旧专题归档、周期反思。

全局确认面 = Desktop **AI 工作区建议 pane**（状态栏计数 chip 打开；专注模式浮动 SuggestPopover）。状态栏显示 AI 工作中 / 后台任务 / 待办整理。
...
**6 条核心规约**（见 `PROJECT-MODEL.md` §3）：大类不重叠 / 专题自然涌现 / 动态类特殊 / 兜底类清理 / 参考资料定位 / 类别命名稳定。

- **口语化**：「记一下」而非工具命令名  

- **三种状态**：Loading / Success / Error（可重试 + 诊断）  
- **证据可见**：保存路径 + 可在 Finder 定位  
- **口语词**：类别 · 专题 · 笔记 · 动态 · 我的情况 · 交付  
